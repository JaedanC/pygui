...
def main():
    ...
    # icon_ranges = pygui.ImGlyphRange([(0xf000, 0xf3ff)])
    default = io.fonts.add_font_default()
    

    # Custom font
    config = pygui.ImFontConfig.create()
    config.merge_mode = True
    # font = io.fonts.add_font_default(config)
    font = io.fonts.add_font_from_file_ttf("pygui/fonts/ProggyClean.ttf", 13, None, io.fonts.get_glyph_ranges_cyrillic())
    ...
    for start, end in ranges.ranges:
        assert start <= end, "wtf steve"

    # io.fonts.add_font_from_file_ttf("pygui/fonts/Font Awesome 6 Free-Regular-400.otf", 13, config, ranges)
    # io.fonts.add_font_from_file_ttf("pygui/fonts/NotoSansJP-Medium.ttf", 13, config, ranges)
    # io.fonts.add_font_from_file_ttf("pygui/fonts/consola.ttf", 13, config, ranges)
    io.fonts.add_font_from_file_ttf("pygui/fonts/CascadiaMono.ttf", 20, config, ranges)
    # for start, end in ranges.ranges:
    #     assert start <= end, "wtf steve"
    ...
    # awesome = io.fonts.add_font_from_file_ttf("pygui/fonts/Font Awesome 6 Free-Regular-400.otf", 18)
    # noto = io.fonts.add_font_from_file_ttf("pygui/fonts/NotoEmoji-VariableFont_wght.ttf", 18)


    # fonts = [ default, karla, noto ]
    # font_names = [ "default", "karla", "noto" ]
    ...
    try:
        while not glfw.window_should_close(window):
            ...
            if pygui.begin("Custom fonts"):
                # pygui.list_box("Use font", use_font, font_names, len(fonts))
                # selected_font = fonts[use_font.value]
                ...
                pygui.push_font(font if use_font else default)
                pygui.text("After push こんにちは！テスト")
                pygui.text("©땔땕땗")
                pygui.text(utf8_test)
                # for i in range(0, len(utf8_test) // 200):
                #     pygui.text(utf8_test[i * 200 : i * 200 + 200])
