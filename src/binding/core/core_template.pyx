...
# ?custom_comment_only(False)
# ?returns(None)
def image_with_bg(tex_ref: ImTextureRef, image_size: Tuple[float, float], uv0: Tuple[float, float]=(0, 0), uv1: Tuple[float, float]=(1, 1), bg_col: Tuple[float, float, float, float]=(0, 0, 0, 0), tint_col: Tuple[float, float, float, float]=(1, 1, 1, 1)):
    """
    Implied uv0 = imvec2(0, 0), uv1 = imvec2(1, 1), bg_col = imvec4(0, 0, 0, 0), tint_col = imvec4(1, 1, 1, 1)
    """
...
cdef class ImDrawList:
    ...
    # ?custom_comment_only(False)
    # ?returns(None)
    def add_image(self: ImDrawList, tex_ref: ImTextureRef, p_min: Tuple[float, float], p_max: Tuple[float, float], uv_min: tuple=(0, 0), uv_max: tuple=(1, 1), col: int=IM_COL32_WHITE):
        """
        Image primitives
    ...
    # ?custom_comment_only(False)
    # ?returns(None)
    def add_image_quad(self: ImDrawList, tex_ref: ImTextureRef, p1: Tuple[float, float], p2: Tuple[float, float], p3: Tuple[float, float], p4: Tuple[float, float], uv1: tuple=(0, 0), uv2: tuple=(1, 0), uv3: tuple=(1, 1), uv4: tuple=(0, 1), col: int=IM_COL32_WHITE):
...
cdef class ImFontAtlas:
    ...
    # ?custom_comment_only(False)
    # ?returns(ImFont)
    def add_font_from_file_ttf(self: ImFontAtlas, filename: str, size_pixels: float=0.0, font_cfg: ImFontConfig=None, glyph_ranges: ImGlyphRange=None):
        """
...
cdef class ImTextureRef:
    ...
    def tex_data(self):
        ...
        return ImTextureData.from_ptr(res) if res != NULL else None
    @tex_data.setter
    def tex_data(self, value: ImTextureData):
    ...
    # ?returns(ImTextureRef)
    @staticmethod
    def create(tex_id: int):
        """
        Create a dynamically allocated instance of ImTextureRef using in id.
        Must also be freed with destroy().
        """
        cdef dcimgui.ImTextureRef* tex_ref = <dcimgui.ImTextureRef*>dcimgui.ImGui_MemAlloc(sizeof(dcimgui.ImTextureRef))
        memset(tex_ref, 0, sizeof(dcimgui.ImTextureRef))
        tex_ref._TexID = tex_id
        return ImTextureRef.from_heap_ptr(tex_ref)
    # [End Method]
