...
def impl_open_gl_3_init(glsl_version: str=None):
    ...
    if glsl_version is None:
        res = ccimgui_opengl3.ImGui_ImplOpenGL3_Init(NULL)
    else:
        res = ccimgui_opengl3.ImGui_ImplOpenGL3_Init(
            _bytes(glsl_version)
        )
    return res
# [End Function]
...
# ?active(True)
# ?returns(None)
def impl_open_gl_3_render_draw_data(draw_data):
    cdef void* ptr = <void*>draw_data._ptr
    ccimgui_opengl3.ImGui_ImplOpenGL3_RenderDrawData(
        <ccimgui_db.ImDrawData*>ptr
    )
# [End Function]
