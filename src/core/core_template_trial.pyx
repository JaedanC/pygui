...
# ?active(True)
# ?returns(Any)
def collapsing_header(label: str, p_visible: BoolPtr=None, flags: int=0):
    """
...
def color_edit4(label: str, colour: Vec4Ptr, flags: int=0):
    ...
    cdef float c_floats[4]
    colour.to_array(c_floats)
    cdef bool res = ccimgui.igColorEdit4(_bytes(label), c_floats, flags)
    colour.from_array(c_floats)
    return res
# [End Function]
...
def combo(label: str, current_item: IntPtr, items: List[str], popup_max_height_in_items: int=-1):
    ...
    """
    # TODO: I'm not happy with the memory management here.
...
# ?active(True)
# ?returns(bool)
def input_int2(label: str, int_ptrs: List[IntPtr], flags: int=0):
    cdef int c_ints[2]
...
# ?active(True)
# ?returns(bool)
def input_int3(label: str, int_ptrs: List[IntPtr], flags: int=0):
    cdef int c_ints[3]
...
# ?active(True)
# ?returns(bool)
def radio_button(label: str, value: IntPtr, v_button: int):
    """
...
def show_demo_window(p_open: BoolPtr=None):
    ...
    if p_open is None:
        ...
        ccimgui.igShowDemoWindow(&p_open.ptr)
# [End Function]
...
def text_unformatted(text: str, text_end: str=None):
    ...
    """
    # I don't think I need to consider text_end
    ccimgui.igTextUnformatted(_bytes(text), NULL)
# [End Function]
...
cdef class ImColor:
    ...
    # ?returns(tuple)
    @staticmethod
    def hsv(h: float, s: float, value: float, a: float=1.0):
        cdef ccimgui.ImColor* color = ccimgui.ImColor_ImColor_Nil()
        ccimgui.ImColor_HSV(color, h, s, value, a)
        vec4 = _cast_ImVec4_tuple(dereference(color).Value)
        ccimgui.ImColor_destroy(color)
        return vec4
    # [End Method]
...
cdef class ImGuiStyle:
    ...
    def item_inner_spacing(self):
        return _cast_ImVec2_tuple(dereference(self._ptr).ItemInnerSpacing)
