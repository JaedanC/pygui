...
cdef _cast_ImVec2_tuple(ccimgui_dear_bindings.ImVec2 vec):
    return (vec.x, vec.y)

cdef ccimgui_dear_bindings.ImVec2 _cast_tuple_ImVec2(pair) except +:
    cdef ccimgui_dear_bindings.ImVec2 vec
    if len(pair) != 2:
        raise ValueError('pair param must be length of 2')
    vec.x, vec.y = pair
    return vec

cdef _cast_ImVec4_tuple(ccimgui_dear_bindings.ImVec4 vec):
    return (vec.x, vec.y, vec.z, vec.w)

cdef ccimgui_dear_bindings.ImVec4 _cast_tuple_ImVec4(quadruple):
    cdef ccimgui_dear_bindings.ImVec4 vec
    if len(quadruple) != 4:
...
def _py_vertex_buffer_vertex_pos_offset():
    return <uintptr_t><size_t>&(<ccimgui_dear_bindings.ImDrawVert*>NULL).pos

def _py_vertex_buffer_vertex_uv_offset():
    return <uintptr_t><size_t>&(<ccimgui_dear_bindings.ImDrawVert*>NULL).uv

def _py_vertex_buffer_vertex_col_offset():
    return <uintptr_t><size_t>&(<ccimgui_dear_bindings.ImDrawVert*>NULL).col

def _py_vertex_buffer_vertex_size():
    return sizeof(ccimgui_dear_bindings.ImDrawVert)

def _py_index_buffer_index_size():
    return sizeof(ccimgui_dear_bindings.ImDrawIdx)
...
cdef class StrPtr:
    ...
    def __init__(self, initial_value: str, buffer_size=256):
        self.buffer = <char*>ccimgui_dear_bindings.igMemAlloc(buffer_size)
        self.buffer_size: int = buffer_size
        self.value = initial_value

    def __dealloc__(self):
        ccimgui_dear_bindings.igMemFree(self.buffer)
...
# [Enums]
WINDOW_FLAGS_NONE = ccimgui_dear_bindings.ImGuiWindowFlags_None
WINDOW_FLAGS_NO_TITLE_BAR = ccimgui_dear_bindings.ImGuiWindowFlags_NoTitleBar
WINDOW_FLAGS_NO_RESIZE = ccimgui_dear_bindings.ImGuiWindowFlags_NoResize
WINDOW_FLAGS_NO_MOVE = ccimgui_dear_bindings.ImGuiWindowFlags_NoMove
WINDOW_FLAGS_NO_SCROLLBAR = ccimgui_dear_bindings.ImGuiWindowFlags_NoScrollbar
WINDOW_FLAGS_NO_SCROLL_WITH_MOUSE = ccimgui_dear_bindings.ImGuiWindowFlags_NoScrollWithMouse
WINDOW_FLAGS_NO_COLLAPSE = ccimgui_dear_bindings.ImGuiWindowFlags_NoCollapse
WINDOW_FLAGS_ALWAYS_AUTO_RESIZE = ccimgui_dear_bindings.ImGuiWindowFlags_AlwaysAutoResize
WINDOW_FLAGS_NO_BACKGROUND = ccimgui_dear_bindings.ImGuiWindowFlags_NoBackground
WINDOW_FLAGS_NO_SAVED_SETTINGS = ccimgui_dear_bindings.ImGuiWindowFlags_NoSavedSettings
WINDOW_FLAGS_NO_MOUSE_INPUTS = ccimgui_dear_bindings.ImGuiWindowFlags_NoMouseInputs
WINDOW_FLAGS_MENU_BAR = ccimgui_dear_bindings.ImGuiWindowFlags_MenuBar
WINDOW_FLAGS_HORIZONTAL_SCROLLBAR = ccimgui_dear_bindings.ImGuiWindowFlags_HorizontalScrollbar
WINDOW_FLAGS_NO_FOCUS_ON_APPEARING = ccimgui_dear_bindings.ImGuiWindowFlags_NoFocusOnAppearing
WINDOW_FLAGS_NO_BRING_TO_FRONT_ON_FOCUS = ccimgui_dear_bindings.ImGuiWindowFlags_NoBringToFrontOnFocus
WINDOW_FLAGS_ALWAYS_VERTICAL_SCROLLBAR = ccimgui_dear_bindings.ImGuiWindowFlags_AlwaysVerticalScrollbar
WINDOW_FLAGS_ALWAYS_HORIZONTAL_SCROLLBAR = ccimgui_dear_bindings.ImGuiWindowFlags_AlwaysHorizontalScrollbar
WINDOW_FLAGS_ALWAYS_USE_WINDOW_PADDING = ccimgui_dear_bindings.ImGuiWindowFlags_AlwaysUseWindowPadding
WINDOW_FLAGS_NO_NAV_INPUTS = ccimgui_dear_bindings.ImGuiWindowFlags_NoNavInputs
WINDOW_FLAGS_NO_NAV_FOCUS = ccimgui_dear_bindings.ImGuiWindowFlags_NoNavFocus
WINDOW_FLAGS_UNSAVED_DOCUMENT = ccimgui_dear_bindings.ImGuiWindowFlags_UnsavedDocument
WINDOW_FLAGS_NO_DOCKING = ccimgui_dear_bindings.ImGuiWindowFlags_NoDocking
WINDOW_FLAGS_NO_NAV = ccimgui_dear_bindings.ImGuiWindowFlags_NoNav
WINDOW_FLAGS_NO_DECORATION = ccimgui_dear_bindings.ImGuiWindowFlags_NoDecoration
WINDOW_FLAGS_NO_INPUTS = ccimgui_dear_bindings.ImGuiWindowFlags_NoInputs
WINDOW_FLAGS_NAV_FLATTENED = ccimgui_dear_bindings.ImGuiWindowFlags_NavFlattened
WINDOW_FLAGS_CHILD_WINDOW = ccimgui_dear_bindings.ImGuiWindowFlags_ChildWindow
WINDOW_FLAGS_TOOLTIP = ccimgui_dear_bindings.ImGuiWindowFlags_Tooltip
WINDOW_FLAGS_POPUP = ccimgui_dear_bindings.ImGuiWindowFlags_Popup
WINDOW_FLAGS_MODAL = ccimgui_dear_bindings.ImGuiWindowFlags_Modal
WINDOW_FLAGS_CHILD_MENU = ccimgui_dear_bindings.ImGuiWindowFlags_ChildMenu
WINDOW_FLAGS_DOCK_NODE_HOST = ccimgui_dear_bindings.ImGuiWindowFlags_DockNodeHost
INPUT_TEXT_FLAGS_NONE = ccimgui_dear_bindings.ImGuiInputTextFlags_None
INPUT_TEXT_FLAGS_CHARS_DECIMAL = ccimgui_dear_bindings.ImGuiInputTextFlags_CharsDecimal
INPUT_TEXT_FLAGS_CHARS_HEXADECIMAL = ccimgui_dear_bindings.ImGuiInputTextFlags_CharsHexadecimal
INPUT_TEXT_FLAGS_CHARS_UPPERCASE = ccimgui_dear_bindings.ImGuiInputTextFlags_CharsUppercase
INPUT_TEXT_FLAGS_CHARS_NO_BLANK = ccimgui_dear_bindings.ImGuiInputTextFlags_CharsNoBlank
INPUT_TEXT_FLAGS_AUTO_SELECT_ALL = ccimgui_dear_bindings.ImGuiInputTextFlags_AutoSelectAll
INPUT_TEXT_FLAGS_ENTER_RETURNS_TRUE = ccimgui_dear_bindings.ImGuiInputTextFlags_EnterReturnsTrue
INPUT_TEXT_FLAGS_CALLBACK_COMPLETION = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackCompletion
INPUT_TEXT_FLAGS_CALLBACK_HISTORY = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackHistory
INPUT_TEXT_FLAGS_CALLBACK_ALWAYS = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackAlways
INPUT_TEXT_FLAGS_CALLBACK_CHAR_FILTER = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackCharFilter
INPUT_TEXT_FLAGS_ALLOW_TAB_INPUT = ccimgui_dear_bindings.ImGuiInputTextFlags_AllowTabInput
INPUT_TEXT_FLAGS_CTRL_ENTER_FOR_NEW_LINE = ccimgui_dear_bindings.ImGuiInputTextFlags_CtrlEnterForNewLine
INPUT_TEXT_FLAGS_NO_HORIZONTAL_SCROLL = ccimgui_dear_bindings.ImGuiInputTextFlags_NoHorizontalScroll
INPUT_TEXT_FLAGS_ALWAYS_OVERWRITE = ccimgui_dear_bindings.ImGuiInputTextFlags_AlwaysOverwrite
INPUT_TEXT_FLAGS_READ_ONLY = ccimgui_dear_bindings.ImGuiInputTextFlags_ReadOnly
INPUT_TEXT_FLAGS_PASSWORD = ccimgui_dear_bindings.ImGuiInputTextFlags_Password
INPUT_TEXT_FLAGS_NO_UNDO_REDO = ccimgui_dear_bindings.ImGuiInputTextFlags_NoUndoRedo
INPUT_TEXT_FLAGS_CHARS_SCIENTIFIC = ccimgui_dear_bindings.ImGuiInputTextFlags_CharsScientific
INPUT_TEXT_FLAGS_CALLBACK_RESIZE = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackResize
INPUT_TEXT_FLAGS_CALLBACK_EDIT = ccimgui_dear_bindings.ImGuiInputTextFlags_CallbackEdit
INPUT_TEXT_FLAGS_ESCAPE_CLEARS_ALL = ccimgui_dear_bindings.ImGuiInputTextFlags_EscapeClearsAll
TREE_NODE_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTreeNodeFlags_None
TREE_NODE_FLAGS_SELECTED = ccimgui_dear_bindings.ImGuiTreeNodeFlags_Selected
TREE_NODE_FLAGS_FRAMED = ccimgui_dear_bindings.ImGuiTreeNodeFlags_Framed
TREE_NODE_FLAGS_ALLOW_ITEM_OVERLAP = ccimgui_dear_bindings.ImGuiTreeNodeFlags_AllowItemOverlap
TREE_NODE_FLAGS_NO_TREE_PUSH_ON_OPEN = ccimgui_dear_bindings.ImGuiTreeNodeFlags_NoTreePushOnOpen
TREE_NODE_FLAGS_NO_AUTO_OPEN_ON_LOG = ccimgui_dear_bindings.ImGuiTreeNodeFlags_NoAutoOpenOnLog
TREE_NODE_FLAGS_DEFAULT_OPEN = ccimgui_dear_bindings.ImGuiTreeNodeFlags_DefaultOpen
TREE_NODE_FLAGS_OPEN_ON_DOUBLE_CLICK = ccimgui_dear_bindings.ImGuiTreeNodeFlags_OpenOnDoubleClick
TREE_NODE_FLAGS_OPEN_ON_ARROW = ccimgui_dear_bindings.ImGuiTreeNodeFlags_OpenOnArrow
TREE_NODE_FLAGS_LEAF = ccimgui_dear_bindings.ImGuiTreeNodeFlags_Leaf
TREE_NODE_FLAGS_BULLET = ccimgui_dear_bindings.ImGuiTreeNodeFlags_Bullet
TREE_NODE_FLAGS_FRAME_PADDING = ccimgui_dear_bindings.ImGuiTreeNodeFlags_FramePadding
TREE_NODE_FLAGS_SPAN_AVAIL_WIDTH = ccimgui_dear_bindings.ImGuiTreeNodeFlags_SpanAvailWidth
TREE_NODE_FLAGS_SPAN_FULL_WIDTH = ccimgui_dear_bindings.ImGuiTreeNodeFlags_SpanFullWidth
TREE_NODE_FLAGS_NAV_LEFT_JUMPS_BACK_HERE = ccimgui_dear_bindings.ImGuiTreeNodeFlags_NavLeftJumpsBackHere
TREE_NODE_FLAGS_COLLAPSING_HEADER = ccimgui_dear_bindings.ImGuiTreeNodeFlags_CollapsingHeader
POPUP_FLAGS_NONE = ccimgui_dear_bindings.ImGuiPopupFlags_None
POPUP_FLAGS_MOUSE_BUTTON_LEFT = ccimgui_dear_bindings.ImGuiPopupFlags_MouseButtonLeft
POPUP_FLAGS_MOUSE_BUTTON_RIGHT = ccimgui_dear_bindings.ImGuiPopupFlags_MouseButtonRight
POPUP_FLAGS_MOUSE_BUTTON_MIDDLE = ccimgui_dear_bindings.ImGuiPopupFlags_MouseButtonMiddle
POPUP_FLAGS_MOUSE_BUTTON_MASK = ccimgui_dear_bindings.ImGuiPopupFlags_MouseButtonMask_
POPUP_FLAGS_MOUSE_BUTTON_DEFAULT = ccimgui_dear_bindings.ImGuiPopupFlags_MouseButtonDefault_
POPUP_FLAGS_NO_OPEN_OVER_EXISTING_POPUP = ccimgui_dear_bindings.ImGuiPopupFlags_NoOpenOverExistingPopup
POPUP_FLAGS_NO_OPEN_OVER_ITEMS = ccimgui_dear_bindings.ImGuiPopupFlags_NoOpenOverItems
POPUP_FLAGS_ANY_POPUP_ID = ccimgui_dear_bindings.ImGuiPopupFlags_AnyPopupId
POPUP_FLAGS_ANY_POPUP_LEVEL = ccimgui_dear_bindings.ImGuiPopupFlags_AnyPopupLevel
POPUP_FLAGS_ANY_POPUP = ccimgui_dear_bindings.ImGuiPopupFlags_AnyPopup
SELECTABLE_FLAGS_NONE = ccimgui_dear_bindings.ImGuiSelectableFlags_None
SELECTABLE_FLAGS_DONT_CLOSE_POPUPS = ccimgui_dear_bindings.ImGuiSelectableFlags_DontClosePopups
SELECTABLE_FLAGS_SPAN_ALL_COLUMNS = ccimgui_dear_bindings.ImGuiSelectableFlags_SpanAllColumns
SELECTABLE_FLAGS_ALLOW_DOUBLE_CLICK = ccimgui_dear_bindings.ImGuiSelectableFlags_AllowDoubleClick
SELECTABLE_FLAGS_DISABLED = ccimgui_dear_bindings.ImGuiSelectableFlags_Disabled
SELECTABLE_FLAGS_ALLOW_ITEM_OVERLAP = ccimgui_dear_bindings.ImGuiSelectableFlags_AllowItemOverlap
COMBO_FLAGS_NONE = ccimgui_dear_bindings.ImGuiComboFlags_None
COMBO_FLAGS_POPUP_ALIGN_LEFT = ccimgui_dear_bindings.ImGuiComboFlags_PopupAlignLeft
COMBO_FLAGS_HEIGHT_SMALL = ccimgui_dear_bindings.ImGuiComboFlags_HeightSmall
COMBO_FLAGS_HEIGHT_REGULAR = ccimgui_dear_bindings.ImGuiComboFlags_HeightRegular
COMBO_FLAGS_HEIGHT_LARGE = ccimgui_dear_bindings.ImGuiComboFlags_HeightLarge
COMBO_FLAGS_HEIGHT_LARGEST = ccimgui_dear_bindings.ImGuiComboFlags_HeightLargest
COMBO_FLAGS_NO_ARROW_BUTTON = ccimgui_dear_bindings.ImGuiComboFlags_NoArrowButton
COMBO_FLAGS_NO_PREVIEW = ccimgui_dear_bindings.ImGuiComboFlags_NoPreview
COMBO_FLAGS_HEIGHT_MASK = ccimgui_dear_bindings.ImGuiComboFlags_HeightMask_
TAB_BAR_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTabBarFlags_None
TAB_BAR_FLAGS_REORDERABLE = ccimgui_dear_bindings.ImGuiTabBarFlags_Reorderable
TAB_BAR_FLAGS_AUTO_SELECT_NEW_TABS = ccimgui_dear_bindings.ImGuiTabBarFlags_AutoSelectNewTabs
TAB_BAR_FLAGS_TAB_LIST_POPUP_BUTTON = ccimgui_dear_bindings.ImGuiTabBarFlags_TabListPopupButton
TAB_BAR_FLAGS_NO_CLOSE_WITH_MIDDLE_MOUSE_BUTTON = ccimgui_dear_bindings.ImGuiTabBarFlags_NoCloseWithMiddleMouseButton
TAB_BAR_FLAGS_NO_TAB_LIST_SCROLLING_BUTTONS = ccimgui_dear_bindings.ImGuiTabBarFlags_NoTabListScrollingButtons
TAB_BAR_FLAGS_NO_TOOLTIP = ccimgui_dear_bindings.ImGuiTabBarFlags_NoTooltip
TAB_BAR_FLAGS_FITTING_POLICY_RESIZE_DOWN = ccimgui_dear_bindings.ImGuiTabBarFlags_FittingPolicyResizeDown
TAB_BAR_FLAGS_FITTING_POLICY_SCROLL = ccimgui_dear_bindings.ImGuiTabBarFlags_FittingPolicyScroll
TAB_BAR_FLAGS_FITTING_POLICY_MASK = ccimgui_dear_bindings.ImGuiTabBarFlags_FittingPolicyMask_
TAB_BAR_FLAGS_FITTING_POLICY_DEFAULT = ccimgui_dear_bindings.ImGuiTabBarFlags_FittingPolicyDefault_
TAB_ITEM_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTabItemFlags_None
TAB_ITEM_FLAGS_UNSAVED_DOCUMENT = ccimgui_dear_bindings.ImGuiTabItemFlags_UnsavedDocument
TAB_ITEM_FLAGS_SET_SELECTED = ccimgui_dear_bindings.ImGuiTabItemFlags_SetSelected
TAB_ITEM_FLAGS_NO_CLOSE_WITH_MIDDLE_MOUSE_BUTTON = ccimgui_dear_bindings.ImGuiTabItemFlags_NoCloseWithMiddleMouseButton
TAB_ITEM_FLAGS_NO_PUSH_ID = ccimgui_dear_bindings.ImGuiTabItemFlags_NoPushId
TAB_ITEM_FLAGS_NO_TOOLTIP = ccimgui_dear_bindings.ImGuiTabItemFlags_NoTooltip
TAB_ITEM_FLAGS_NO_REORDER = ccimgui_dear_bindings.ImGuiTabItemFlags_NoReorder
TAB_ITEM_FLAGS_LEADING = ccimgui_dear_bindings.ImGuiTabItemFlags_Leading
TAB_ITEM_FLAGS_TRAILING = ccimgui_dear_bindings.ImGuiTabItemFlags_Trailing
TABLE_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTableFlags_None
TABLE_FLAGS_RESIZABLE = ccimgui_dear_bindings.ImGuiTableFlags_Resizable
TABLE_FLAGS_REORDERABLE = ccimgui_dear_bindings.ImGuiTableFlags_Reorderable
TABLE_FLAGS_HIDEABLE = ccimgui_dear_bindings.ImGuiTableFlags_Hideable
TABLE_FLAGS_SORTABLE = ccimgui_dear_bindings.ImGuiTableFlags_Sortable
TABLE_FLAGS_NO_SAVED_SETTINGS = ccimgui_dear_bindings.ImGuiTableFlags_NoSavedSettings
TABLE_FLAGS_CONTEXT_MENU_IN_BODY = ccimgui_dear_bindings.ImGuiTableFlags_ContextMenuInBody
TABLE_FLAGS_ROW_BG = ccimgui_dear_bindings.ImGuiTableFlags_RowBg
TABLE_FLAGS_BORDERS_INNER_H = ccimgui_dear_bindings.ImGuiTableFlags_BordersInnerH
TABLE_FLAGS_BORDERS_OUTER_H = ccimgui_dear_bindings.ImGuiTableFlags_BordersOuterH
TABLE_FLAGS_BORDERS_INNER_V = ccimgui_dear_bindings.ImGuiTableFlags_BordersInnerV
TABLE_FLAGS_BORDERS_OUTER_V = ccimgui_dear_bindings.ImGuiTableFlags_BordersOuterV
TABLE_FLAGS_BORDERS_H = ccimgui_dear_bindings.ImGuiTableFlags_BordersH
TABLE_FLAGS_BORDERS_V = ccimgui_dear_bindings.ImGuiTableFlags_BordersV
TABLE_FLAGS_BORDERS_INNER = ccimgui_dear_bindings.ImGuiTableFlags_BordersInner
TABLE_FLAGS_BORDERS_OUTER = ccimgui_dear_bindings.ImGuiTableFlags_BordersOuter
TABLE_FLAGS_BORDERS = ccimgui_dear_bindings.ImGuiTableFlags_Borders
TABLE_FLAGS_NO_BORDERS_IN_BODY = ccimgui_dear_bindings.ImGuiTableFlags_NoBordersInBody
TABLE_FLAGS_NO_BORDERS_IN_BODY_UNTIL_RESIZE = ccimgui_dear_bindings.ImGuiTableFlags_NoBordersInBodyUntilResize
TABLE_FLAGS_SIZING_FIXED_FIT = ccimgui_dear_bindings.ImGuiTableFlags_SizingFixedFit
TABLE_FLAGS_SIZING_FIXED_SAME = ccimgui_dear_bindings.ImGuiTableFlags_SizingFixedSame
TABLE_FLAGS_SIZING_STRETCH_PROP = ccimgui_dear_bindings.ImGuiTableFlags_SizingStretchProp
TABLE_FLAGS_SIZING_STRETCH_SAME = ccimgui_dear_bindings.ImGuiTableFlags_SizingStretchSame
TABLE_FLAGS_NO_HOST_EXTEND_X = ccimgui_dear_bindings.ImGuiTableFlags_NoHostExtendX
TABLE_FLAGS_NO_HOST_EXTEND_Y = ccimgui_dear_bindings.ImGuiTableFlags_NoHostExtendY
TABLE_FLAGS_NO_KEEP_COLUMNS_VISIBLE = ccimgui_dear_bindings.ImGuiTableFlags_NoKeepColumnsVisible
TABLE_FLAGS_PRECISE_WIDTHS = ccimgui_dear_bindings.ImGuiTableFlags_PreciseWidths
TABLE_FLAGS_NO_CLIP = ccimgui_dear_bindings.ImGuiTableFlags_NoClip
TABLE_FLAGS_PAD_OUTER_X = ccimgui_dear_bindings.ImGuiTableFlags_PadOuterX
TABLE_FLAGS_NO_PAD_OUTER_X = ccimgui_dear_bindings.ImGuiTableFlags_NoPadOuterX
TABLE_FLAGS_NO_PAD_INNER_X = ccimgui_dear_bindings.ImGuiTableFlags_NoPadInnerX
TABLE_FLAGS_SCROLL_X = ccimgui_dear_bindings.ImGuiTableFlags_ScrollX
TABLE_FLAGS_SCROLL_Y = ccimgui_dear_bindings.ImGuiTableFlags_ScrollY
TABLE_FLAGS_SORT_MULTI = ccimgui_dear_bindings.ImGuiTableFlags_SortMulti
TABLE_FLAGS_SORT_TRISTATE = ccimgui_dear_bindings.ImGuiTableFlags_SortTristate
TABLE_FLAGS_SIZING_MASK = ccimgui_dear_bindings.ImGuiTableFlags_SizingMask_
TABLE_COLUMN_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTableColumnFlags_None
TABLE_COLUMN_FLAGS_DISABLED = ccimgui_dear_bindings.ImGuiTableColumnFlags_Disabled
TABLE_COLUMN_FLAGS_DEFAULT_HIDE = ccimgui_dear_bindings.ImGuiTableColumnFlags_DefaultHide
TABLE_COLUMN_FLAGS_DEFAULT_SORT = ccimgui_dear_bindings.ImGuiTableColumnFlags_DefaultSort
TABLE_COLUMN_FLAGS_WIDTH_STRETCH = ccimgui_dear_bindings.ImGuiTableColumnFlags_WidthStretch
TABLE_COLUMN_FLAGS_WIDTH_FIXED = ccimgui_dear_bindings.ImGuiTableColumnFlags_WidthFixed
TABLE_COLUMN_FLAGS_NO_RESIZE = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoResize
TABLE_COLUMN_FLAGS_NO_REORDER = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoReorder
TABLE_COLUMN_FLAGS_NO_HIDE = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoHide
TABLE_COLUMN_FLAGS_NO_CLIP = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoClip
TABLE_COLUMN_FLAGS_NO_SORT = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoSort
TABLE_COLUMN_FLAGS_NO_SORT_ASCENDING = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoSortAscending
TABLE_COLUMN_FLAGS_NO_SORT_DESCENDING = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoSortDescending
TABLE_COLUMN_FLAGS_NO_HEADER_LABEL = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoHeaderLabel
TABLE_COLUMN_FLAGS_NO_HEADER_WIDTH = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoHeaderWidth
TABLE_COLUMN_FLAGS_PREFER_SORT_ASCENDING = ccimgui_dear_bindings.ImGuiTableColumnFlags_PreferSortAscending
TABLE_COLUMN_FLAGS_PREFER_SORT_DESCENDING = ccimgui_dear_bindings.ImGuiTableColumnFlags_PreferSortDescending
TABLE_COLUMN_FLAGS_INDENT_ENABLE = ccimgui_dear_bindings.ImGuiTableColumnFlags_IndentEnable
TABLE_COLUMN_FLAGS_INDENT_DISABLE = ccimgui_dear_bindings.ImGuiTableColumnFlags_IndentDisable
TABLE_COLUMN_FLAGS_IS_ENABLED = ccimgui_dear_bindings.ImGuiTableColumnFlags_IsEnabled
TABLE_COLUMN_FLAGS_IS_VISIBLE = ccimgui_dear_bindings.ImGuiTableColumnFlags_IsVisible
TABLE_COLUMN_FLAGS_IS_SORTED = ccimgui_dear_bindings.ImGuiTableColumnFlags_IsSorted
TABLE_COLUMN_FLAGS_IS_HOVERED = ccimgui_dear_bindings.ImGuiTableColumnFlags_IsHovered
TABLE_COLUMN_FLAGS_WIDTH_MASK = ccimgui_dear_bindings.ImGuiTableColumnFlags_WidthMask_
TABLE_COLUMN_FLAGS_INDENT_MASK = ccimgui_dear_bindings.ImGuiTableColumnFlags_IndentMask_
TABLE_COLUMN_FLAGS_STATUS_MASK = ccimgui_dear_bindings.ImGuiTableColumnFlags_StatusMask_
TABLE_COLUMN_FLAGS_NO_DIRECT_RESIZE = ccimgui_dear_bindings.ImGuiTableColumnFlags_NoDirectResize_
TABLE_ROW_FLAGS_NONE = ccimgui_dear_bindings.ImGuiTableRowFlags_None
TABLE_ROW_FLAGS_HEADERS = ccimgui_dear_bindings.ImGuiTableRowFlags_Headers
TABLE_BG_TARGET_NONE = ccimgui_dear_bindings.ImGuiTableBgTarget_None
TABLE_BG_TARGET_ROW_BG0 = ccimgui_dear_bindings.ImGuiTableBgTarget_RowBg0
TABLE_BG_TARGET_ROW_BG1 = ccimgui_dear_bindings.ImGuiTableBgTarget_RowBg1
TABLE_BG_TARGET_CELL_BG = ccimgui_dear_bindings.ImGuiTableBgTarget_CellBg
FOCUSED_FLAGS_NONE = ccimgui_dear_bindings.ImGuiFocusedFlags_None
FOCUSED_FLAGS_CHILD_WINDOWS = ccimgui_dear_bindings.ImGuiFocusedFlags_ChildWindows
FOCUSED_FLAGS_ROOT_WINDOW = ccimgui_dear_bindings.ImGuiFocusedFlags_RootWindow
FOCUSED_FLAGS_ANY_WINDOW = ccimgui_dear_bindings.ImGuiFocusedFlags_AnyWindow
FOCUSED_FLAGS_NO_POPUP_HIERARCHY = ccimgui_dear_bindings.ImGuiFocusedFlags_NoPopupHierarchy
FOCUSED_FLAGS_DOCK_HIERARCHY = ccimgui_dear_bindings.ImGuiFocusedFlags_DockHierarchy
FOCUSED_FLAGS_ROOT_AND_CHILD_WINDOWS = ccimgui_dear_bindings.ImGuiFocusedFlags_RootAndChildWindows
HOVERED_FLAGS_NONE = ccimgui_dear_bindings.ImGuiHoveredFlags_None
HOVERED_FLAGS_CHILD_WINDOWS = ccimgui_dear_bindings.ImGuiHoveredFlags_ChildWindows
HOVERED_FLAGS_ROOT_WINDOW = ccimgui_dear_bindings.ImGuiHoveredFlags_RootWindow
HOVERED_FLAGS_ANY_WINDOW = ccimgui_dear_bindings.ImGuiHoveredFlags_AnyWindow
HOVERED_FLAGS_NO_POPUP_HIERARCHY = ccimgui_dear_bindings.ImGuiHoveredFlags_NoPopupHierarchy
HOVERED_FLAGS_DOCK_HIERARCHY = ccimgui_dear_bindings.ImGuiHoveredFlags_DockHierarchy
HOVERED_FLAGS_ALLOW_WHEN_BLOCKED_BY_POPUP = ccimgui_dear_bindings.ImGuiHoveredFlags_AllowWhenBlockedByPopup
HOVERED_FLAGS_ALLOW_WHEN_BLOCKED_BY_ACTIVE_ITEM = ccimgui_dear_bindings.ImGuiHoveredFlags_AllowWhenBlockedByActiveItem
HOVERED_FLAGS_ALLOW_WHEN_OVERLAPPED = ccimgui_dear_bindings.ImGuiHoveredFlags_AllowWhenOverlapped
HOVERED_FLAGS_ALLOW_WHEN_DISABLED = ccimgui_dear_bindings.ImGuiHoveredFlags_AllowWhenDisabled
HOVERED_FLAGS_NO_NAV_OVERRIDE = ccimgui_dear_bindings.ImGuiHoveredFlags_NoNavOverride
HOVERED_FLAGS_RECT_ONLY = ccimgui_dear_bindings.ImGuiHoveredFlags_RectOnly
HOVERED_FLAGS_ROOT_AND_CHILD_WINDOWS = ccimgui_dear_bindings.ImGuiHoveredFlags_RootAndChildWindows
HOVERED_FLAGS_DELAY_NORMAL = ccimgui_dear_bindings.ImGuiHoveredFlags_DelayNormal
HOVERED_FLAGS_DELAY_SHORT = ccimgui_dear_bindings.ImGuiHoveredFlags_DelayShort
HOVERED_FLAGS_NO_SHARED_DELAY = ccimgui_dear_bindings.ImGuiHoveredFlags_NoSharedDelay
DOCK_NODE_FLAGS_NONE = ccimgui_dear_bindings.ImGuiDockNodeFlags_None
DOCK_NODE_FLAGS_KEEP_ALIVE_ONLY = ccimgui_dear_bindings.ImGuiDockNodeFlags_KeepAliveOnly
DOCK_NODE_FLAGS_NO_DOCKING_IN_CENTRAL_NODE = ccimgui_dear_bindings.ImGuiDockNodeFlags_NoDockingInCentralNode
DOCK_NODE_FLAGS_PASSTHRU_CENTRAL_NODE = ccimgui_dear_bindings.ImGuiDockNodeFlags_PassthruCentralNode
DOCK_NODE_FLAGS_NO_SPLIT = ccimgui_dear_bindings.ImGuiDockNodeFlags_NoSplit
DOCK_NODE_FLAGS_NO_RESIZE = ccimgui_dear_bindings.ImGuiDockNodeFlags_NoResize
DOCK_NODE_FLAGS_AUTO_HIDE_TAB_BAR = ccimgui_dear_bindings.ImGuiDockNodeFlags_AutoHideTabBar
DRAG_DROP_FLAGS_NONE = ccimgui_dear_bindings.ImGuiDragDropFlags_None
DRAG_DROP_FLAGS_SOURCE_NO_PREVIEW_TOOLTIP = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceNoPreviewTooltip
DRAG_DROP_FLAGS_SOURCE_NO_DISABLE_HOVER = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceNoDisableHover
DRAG_DROP_FLAGS_SOURCE_NO_HOLD_TO_OPEN_OTHERS = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceNoHoldToOpenOthers
DRAG_DROP_FLAGS_SOURCE_ALLOW_NULL_ID = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceAllowNullID
DRAG_DROP_FLAGS_SOURCE_EXTERN = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceExtern
DRAG_DROP_FLAGS_SOURCE_AUTO_EXPIRE_PAYLOAD = ccimgui_dear_bindings.ImGuiDragDropFlags_SourceAutoExpirePayload
DRAG_DROP_FLAGS_ACCEPT_BEFORE_DELIVERY = ccimgui_dear_bindings.ImGuiDragDropFlags_AcceptBeforeDelivery
DRAG_DROP_FLAGS_ACCEPT_NO_DRAW_DEFAULT_RECT = ccimgui_dear_bindings.ImGuiDragDropFlags_AcceptNoDrawDefaultRect
DRAG_DROP_FLAGS_ACCEPT_NO_PREVIEW_TOOLTIP = ccimgui_dear_bindings.ImGuiDragDropFlags_AcceptNoPreviewTooltip
DRAG_DROP_FLAGS_ACCEPT_PEEK_ONLY = ccimgui_dear_bindings.ImGuiDragDropFlags_AcceptPeekOnly
DATA_TYPE_S8 = ccimgui_dear_bindings.ImGuiDataType_S8
DATA_TYPE_U8 = ccimgui_dear_bindings.ImGuiDataType_U8
DATA_TYPE_S16 = ccimgui_dear_bindings.ImGuiDataType_S16
DATA_TYPE_U16 = ccimgui_dear_bindings.ImGuiDataType_U16
DATA_TYPE_S32 = ccimgui_dear_bindings.ImGuiDataType_S32
DATA_TYPE_U32 = ccimgui_dear_bindings.ImGuiDataType_U32
DATA_TYPE_S64 = ccimgui_dear_bindings.ImGuiDataType_S64
DATA_TYPE_U64 = ccimgui_dear_bindings.ImGuiDataType_U64
DATA_TYPE_FLOAT = ccimgui_dear_bindings.ImGuiDataType_Float
DATA_TYPE_DOUBLE = ccimgui_dear_bindings.ImGuiDataType_Double
DATA_TYPE_COUNT = ccimgui_dear_bindings.ImGuiDataType_COUNT
DIR_NONE = ccimgui_dear_bindings.ImGuiDir_None
DIR_LEFT = ccimgui_dear_bindings.ImGuiDir_Left
DIR_RIGHT = ccimgui_dear_bindings.ImGuiDir_Right
DIR_UP = ccimgui_dear_bindings.ImGuiDir_Up
DIR_DOWN = ccimgui_dear_bindings.ImGuiDir_Down
DIR_COUNT = ccimgui_dear_bindings.ImGuiDir_COUNT
SORT_DIRECTION_NONE = ccimgui_dear_bindings.ImGuiSortDirection_None
SORT_DIRECTION_ASCENDING = ccimgui_dear_bindings.ImGuiSortDirection_Ascending
SORT_DIRECTION_DESCENDING = ccimgui_dear_bindings.ImGuiSortDirection_Descending
KEY_NONE = ccimgui_dear_bindings.ImGuiKey_None
KEY_TAB = ccimgui_dear_bindings.ImGuiKey_Tab
KEY_LEFT_ARROW = ccimgui_dear_bindings.ImGuiKey_LeftArrow
KEY_RIGHT_ARROW = ccimgui_dear_bindings.ImGuiKey_RightArrow
KEY_UP_ARROW = ccimgui_dear_bindings.ImGuiKey_UpArrow
KEY_DOWN_ARROW = ccimgui_dear_bindings.ImGuiKey_DownArrow
KEY_PAGE_UP = ccimgui_dear_bindings.ImGuiKey_PageUp
KEY_PAGE_DOWN = ccimgui_dear_bindings.ImGuiKey_PageDown
KEY_HOME = ccimgui_dear_bindings.ImGuiKey_Home
KEY_END = ccimgui_dear_bindings.ImGuiKey_End
KEY_INSERT = ccimgui_dear_bindings.ImGuiKey_Insert
KEY_DELETE = ccimgui_dear_bindings.ImGuiKey_Delete
KEY_BACKSPACE = ccimgui_dear_bindings.ImGuiKey_Backspace
KEY_SPACE = ccimgui_dear_bindings.ImGuiKey_Space
KEY_ENTER = ccimgui_dear_bindings.ImGuiKey_Enter
KEY_ESCAPE = ccimgui_dear_bindings.ImGuiKey_Escape
KEY_LEFT_CTRL = ccimgui_dear_bindings.ImGuiKey_LeftCtrl
KEY_LEFT_SHIFT = ccimgui_dear_bindings.ImGuiKey_LeftShift
KEY_LEFT_ALT = ccimgui_dear_bindings.ImGuiKey_LeftAlt
KEY_LEFT_SUPER = ccimgui_dear_bindings.ImGuiKey_LeftSuper
KEY_RIGHT_CTRL = ccimgui_dear_bindings.ImGuiKey_RightCtrl
KEY_RIGHT_SHIFT = ccimgui_dear_bindings.ImGuiKey_RightShift
KEY_RIGHT_ALT = ccimgui_dear_bindings.ImGuiKey_RightAlt
KEY_RIGHT_SUPER = ccimgui_dear_bindings.ImGuiKey_RightSuper
KEY_MENU = ccimgui_dear_bindings.ImGuiKey_Menu
KEY0 = ccimgui_dear_bindings.ImGuiKey_0
KEY1 = ccimgui_dear_bindings.ImGuiKey_1
KEY2 = ccimgui_dear_bindings.ImGuiKey_2
KEY3 = ccimgui_dear_bindings.ImGuiKey_3
KEY4 = ccimgui_dear_bindings.ImGuiKey_4
KEY5 = ccimgui_dear_bindings.ImGuiKey_5
KEY6 = ccimgui_dear_bindings.ImGuiKey_6
KEY7 = ccimgui_dear_bindings.ImGuiKey_7
KEY8 = ccimgui_dear_bindings.ImGuiKey_8
KEY9 = ccimgui_dear_bindings.ImGuiKey_9
KEY_A = ccimgui_dear_bindings.ImGuiKey_A
KEY_B = ccimgui_dear_bindings.ImGuiKey_B
KEY_C = ccimgui_dear_bindings.ImGuiKey_C
KEY_D = ccimgui_dear_bindings.ImGuiKey_D
KEY_E = ccimgui_dear_bindings.ImGuiKey_E
KEY_F = ccimgui_dear_bindings.ImGuiKey_F
KEY_G = ccimgui_dear_bindings.ImGuiKey_G
KEY_H = ccimgui_dear_bindings.ImGuiKey_H
KEY_I = ccimgui_dear_bindings.ImGuiKey_I
KEY_J = ccimgui_dear_bindings.ImGuiKey_J
KEY_K = ccimgui_dear_bindings.ImGuiKey_K
KEY_L = ccimgui_dear_bindings.ImGuiKey_L
KEY_M = ccimgui_dear_bindings.ImGuiKey_M
KEY_N = ccimgui_dear_bindings.ImGuiKey_N
KEY_O = ccimgui_dear_bindings.ImGuiKey_O
KEY_P = ccimgui_dear_bindings.ImGuiKey_P
KEY_Q = ccimgui_dear_bindings.ImGuiKey_Q
KEY_R = ccimgui_dear_bindings.ImGuiKey_R
KEY_S = ccimgui_dear_bindings.ImGuiKey_S
KEY_T = ccimgui_dear_bindings.ImGuiKey_T
KEY_U = ccimgui_dear_bindings.ImGuiKey_U
KEY_V = ccimgui_dear_bindings.ImGuiKey_V
KEY_W = ccimgui_dear_bindings.ImGuiKey_W
KEY_X = ccimgui_dear_bindings.ImGuiKey_X
KEY_Y = ccimgui_dear_bindings.ImGuiKey_Y
KEY_Z = ccimgui_dear_bindings.ImGuiKey_Z
KEY_F1 = ccimgui_dear_bindings.ImGuiKey_F1
KEY_F2 = ccimgui_dear_bindings.ImGuiKey_F2
KEY_F3 = ccimgui_dear_bindings.ImGuiKey_F3
KEY_F4 = ccimgui_dear_bindings.ImGuiKey_F4
KEY_F5 = ccimgui_dear_bindings.ImGuiKey_F5
KEY_F6 = ccimgui_dear_bindings.ImGuiKey_F6
KEY_F7 = ccimgui_dear_bindings.ImGuiKey_F7
KEY_F8 = ccimgui_dear_bindings.ImGuiKey_F8
KEY_F9 = ccimgui_dear_bindings.ImGuiKey_F9
KEY_F10 = ccimgui_dear_bindings.ImGuiKey_F10
KEY_F11 = ccimgui_dear_bindings.ImGuiKey_F11
KEY_F12 = ccimgui_dear_bindings.ImGuiKey_F12
KEY_APOSTROPHE = ccimgui_dear_bindings.ImGuiKey_Apostrophe
KEY_COMMA = ccimgui_dear_bindings.ImGuiKey_Comma
KEY_MINUS = ccimgui_dear_bindings.ImGuiKey_Minus
KEY_PERIOD = ccimgui_dear_bindings.ImGuiKey_Period
KEY_SLASH = ccimgui_dear_bindings.ImGuiKey_Slash
KEY_SEMICOLON = ccimgui_dear_bindings.ImGuiKey_Semicolon
KEY_EQUAL = ccimgui_dear_bindings.ImGuiKey_Equal
KEY_LEFT_BRACKET = ccimgui_dear_bindings.ImGuiKey_LeftBracket
KEY_BACKSLASH = ccimgui_dear_bindings.ImGuiKey_Backslash
KEY_RIGHT_BRACKET = ccimgui_dear_bindings.ImGuiKey_RightBracket
KEY_GRAVE_ACCENT = ccimgui_dear_bindings.ImGuiKey_GraveAccent
KEY_CAPS_LOCK = ccimgui_dear_bindings.ImGuiKey_CapsLock
KEY_SCROLL_LOCK = ccimgui_dear_bindings.ImGuiKey_ScrollLock
KEY_NUM_LOCK = ccimgui_dear_bindings.ImGuiKey_NumLock
KEY_PRINT_SCREEN = ccimgui_dear_bindings.ImGuiKey_PrintScreen
KEY_PAUSE = ccimgui_dear_bindings.ImGuiKey_Pause
KEY_KEYPAD0 = ccimgui_dear_bindings.ImGuiKey_Keypad0
KEY_KEYPAD1 = ccimgui_dear_bindings.ImGuiKey_Keypad1
KEY_KEYPAD2 = ccimgui_dear_bindings.ImGuiKey_Keypad2
KEY_KEYPAD3 = ccimgui_dear_bindings.ImGuiKey_Keypad3
KEY_KEYPAD4 = ccimgui_dear_bindings.ImGuiKey_Keypad4
KEY_KEYPAD5 = ccimgui_dear_bindings.ImGuiKey_Keypad5
KEY_KEYPAD6 = ccimgui_dear_bindings.ImGuiKey_Keypad6
KEY_KEYPAD7 = ccimgui_dear_bindings.ImGuiKey_Keypad7
KEY_KEYPAD8 = ccimgui_dear_bindings.ImGuiKey_Keypad8
KEY_KEYPAD9 = ccimgui_dear_bindings.ImGuiKey_Keypad9
KEY_KEYPAD_DECIMAL = ccimgui_dear_bindings.ImGuiKey_KeypadDecimal
KEY_KEYPAD_DIVIDE = ccimgui_dear_bindings.ImGuiKey_KeypadDivide
KEY_KEYPAD_MULTIPLY = ccimgui_dear_bindings.ImGuiKey_KeypadMultiply
KEY_KEYPAD_SUBTRACT = ccimgui_dear_bindings.ImGuiKey_KeypadSubtract
KEY_KEYPAD_ADD = ccimgui_dear_bindings.ImGuiKey_KeypadAdd
KEY_KEYPAD_ENTER = ccimgui_dear_bindings.ImGuiKey_KeypadEnter
KEY_KEYPAD_EQUAL = ccimgui_dear_bindings.ImGuiKey_KeypadEqual
KEY_GAMEPAD_START = ccimgui_dear_bindings.ImGuiKey_GamepadStart
KEY_GAMEPAD_BACK = ccimgui_dear_bindings.ImGuiKey_GamepadBack
KEY_GAMEPAD_FACE_LEFT = ccimgui_dear_bindings.ImGuiKey_GamepadFaceLeft
KEY_GAMEPAD_FACE_RIGHT = ccimgui_dear_bindings.ImGuiKey_GamepadFaceRight
KEY_GAMEPAD_FACE_UP = ccimgui_dear_bindings.ImGuiKey_GamepadFaceUp
KEY_GAMEPAD_FACE_DOWN = ccimgui_dear_bindings.ImGuiKey_GamepadFaceDown
KEY_GAMEPAD_DPAD_LEFT = ccimgui_dear_bindings.ImGuiKey_GamepadDpadLeft
KEY_GAMEPAD_DPAD_RIGHT = ccimgui_dear_bindings.ImGuiKey_GamepadDpadRight
KEY_GAMEPAD_DPAD_UP = ccimgui_dear_bindings.ImGuiKey_GamepadDpadUp
KEY_GAMEPAD_DPAD_DOWN = ccimgui_dear_bindings.ImGuiKey_GamepadDpadDown
KEY_GAMEPAD_L1 = ccimgui_dear_bindings.ImGuiKey_GamepadL1
KEY_GAMEPAD_R1 = ccimgui_dear_bindings.ImGuiKey_GamepadR1
KEY_GAMEPAD_L2 = ccimgui_dear_bindings.ImGuiKey_GamepadL2
KEY_GAMEPAD_R2 = ccimgui_dear_bindings.ImGuiKey_GamepadR2
KEY_GAMEPAD_L3 = ccimgui_dear_bindings.ImGuiKey_GamepadL3
KEY_GAMEPAD_R3 = ccimgui_dear_bindings.ImGuiKey_GamepadR3
KEY_GAMEPAD_LS_TICK_LEFT = ccimgui_dear_bindings.ImGuiKey_GamepadLStickLeft
KEY_GAMEPAD_LS_TICK_RIGHT = ccimgui_dear_bindings.ImGuiKey_GamepadLStickRight
KEY_GAMEPAD_LS_TICK_UP = ccimgui_dear_bindings.ImGuiKey_GamepadLStickUp
KEY_GAMEPAD_LS_TICK_DOWN = ccimgui_dear_bindings.ImGuiKey_GamepadLStickDown
KEY_GAMEPAD_RS_TICK_LEFT = ccimgui_dear_bindings.ImGuiKey_GamepadRStickLeft
KEY_GAMEPAD_RS_TICK_RIGHT = ccimgui_dear_bindings.ImGuiKey_GamepadRStickRight
KEY_GAMEPAD_RS_TICK_UP = ccimgui_dear_bindings.ImGuiKey_GamepadRStickUp
KEY_GAMEPAD_RS_TICK_DOWN = ccimgui_dear_bindings.ImGuiKey_GamepadRStickDown
KEY_MOUSE_LEFT = ccimgui_dear_bindings.ImGuiKey_MouseLeft
KEY_MOUSE_RIGHT = ccimgui_dear_bindings.ImGuiKey_MouseRight
KEY_MOUSE_MIDDLE = ccimgui_dear_bindings.ImGuiKey_MouseMiddle
KEY_MOUSE_X1 = ccimgui_dear_bindings.ImGuiKey_MouseX1
KEY_MOUSE_X2 = ccimgui_dear_bindings.ImGuiKey_MouseX2
KEY_MOUSE_WHEEL_X = ccimgui_dear_bindings.ImGuiKey_MouseWheelX
KEY_MOUSE_WHEEL_Y = ccimgui_dear_bindings.ImGuiKey_MouseWheelY
KEY_RESERVED_FOR_MOD_CTRL = ccimgui_dear_bindings.ImGuiKey_ReservedForModCtrl
KEY_RESERVED_FOR_MOD_SHIFT = ccimgui_dear_bindings.ImGuiKey_ReservedForModShift
KEY_RESERVED_FOR_MOD_ALT = ccimgui_dear_bindings.ImGuiKey_ReservedForModAlt
KEY_RESERVED_FOR_MOD_SUPER = ccimgui_dear_bindings.ImGuiKey_ReservedForModSuper
KEY_COUNT = ccimgui_dear_bindings.ImGuiKey_COUNT
MOD_NONE = ccimgui_dear_bindings.ImGuiMod_None
MOD_CTRL = ccimgui_dear_bindings.ImGuiMod_Ctrl
MOD_SHIFT = ccimgui_dear_bindings.ImGuiMod_Shift
MOD_ALT = ccimgui_dear_bindings.ImGuiMod_Alt
MOD_SUPER = ccimgui_dear_bindings.ImGuiMod_Super
MOD_SHORTCUT = ccimgui_dear_bindings.ImGuiMod_Shortcut
MOD_MASK = ccimgui_dear_bindings.ImGuiMod_Mask_
KEY_NAMED_KEY_BEGIN = ccimgui_dear_bindings.ImGuiKey_NamedKey_BEGIN
KEY_NAMED_KEY_END = ccimgui_dear_bindings.ImGuiKey_NamedKey_END
KEY_NAMED_KEY_COUNT = ccimgui_dear_bindings.ImGuiKey_NamedKey_COUNT
KEY_KEYS_DATA_SIZE = ccimgui_dear_bindings.ImGuiKey_KeysData_SIZE
KEY_KEYS_DATA_OFFSET = ccimgui_dear_bindings.ImGuiKey_KeysData_OFFSET
CONFIG_FLAGS_NONE = ccimgui_dear_bindings.ImGuiConfigFlags_None
CONFIG_FLAGS_NAV_ENABLE_KEYBOARD = ccimgui_dear_bindings.ImGuiConfigFlags_NavEnableKeyboard
CONFIG_FLAGS_NAV_ENABLE_GAMEPAD = ccimgui_dear_bindings.ImGuiConfigFlags_NavEnableGamepad
CONFIG_FLAGS_NAV_ENABLE_SET_MOUSE_POS = ccimgui_dear_bindings.ImGuiConfigFlags_NavEnableSetMousePos
CONFIG_FLAGS_NAV_NO_CAPTURE_KEYBOARD = ccimgui_dear_bindings.ImGuiConfigFlags_NavNoCaptureKeyboard
CONFIG_FLAGS_NO_MOUSE = ccimgui_dear_bindings.ImGuiConfigFlags_NoMouse
CONFIG_FLAGS_NO_MOUSE_CURSOR_CHANGE = ccimgui_dear_bindings.ImGuiConfigFlags_NoMouseCursorChange
CONFIG_FLAGS_DOCKING_ENABLE = ccimgui_dear_bindings.ImGuiConfigFlags_DockingEnable
CONFIG_FLAGS_VIEWPORTS_ENABLE = ccimgui_dear_bindings.ImGuiConfigFlags_ViewportsEnable
CONFIG_FLAGS_DPI_ENABLE_SCALE_VIEWPORTS = ccimgui_dear_bindings.ImGuiConfigFlags_DpiEnableScaleViewports
CONFIG_FLAGS_DPI_ENABLE_SCALE_FONTS = ccimgui_dear_bindings.ImGuiConfigFlags_DpiEnableScaleFonts
CONFIG_FLAGS_IS_SRGB = ccimgui_dear_bindings.ImGuiConfigFlags_IsSRGB
CONFIG_FLAGS_IS_TOUCH_SCREEN = ccimgui_dear_bindings.ImGuiConfigFlags_IsTouchScreen
BACKEND_FLAGS_NONE = ccimgui_dear_bindings.ImGuiBackendFlags_None
BACKEND_FLAGS_HAS_GAMEPAD = ccimgui_dear_bindings.ImGuiBackendFlags_HasGamepad
BACKEND_FLAGS_HAS_MOUSE_CURSORS = ccimgui_dear_bindings.ImGuiBackendFlags_HasMouseCursors
BACKEND_FLAGS_HAS_SET_MOUSE_POS = ccimgui_dear_bindings.ImGuiBackendFlags_HasSetMousePos
BACKEND_FLAGS_RENDERER_HAS_VTX_OFFSET = ccimgui_dear_bindings.ImGuiBackendFlags_RendererHasVtxOffset
BACKEND_FLAGS_PLATFORM_HAS_VIEWPORTS = ccimgui_dear_bindings.ImGuiBackendFlags_PlatformHasViewports
BACKEND_FLAGS_HAS_MOUSE_HOVERED_VIEWPORT = ccimgui_dear_bindings.ImGuiBackendFlags_HasMouseHoveredViewport
BACKEND_FLAGS_RENDERER_HAS_VIEWPORTS = ccimgui_dear_bindings.ImGuiBackendFlags_RendererHasViewports
COL_TEXT = ccimgui_dear_bindings.ImGuiCol_Text
COL_TEXT_DISABLED = ccimgui_dear_bindings.ImGuiCol_TextDisabled
COL_WINDOW_BG = ccimgui_dear_bindings.ImGuiCol_WindowBg
COL_CHILD_BG = ccimgui_dear_bindings.ImGuiCol_ChildBg
COL_POPUP_BG = ccimgui_dear_bindings.ImGuiCol_PopupBg
COL_BORDER = ccimgui_dear_bindings.ImGuiCol_Border
COL_BORDER_SHADOW = ccimgui_dear_bindings.ImGuiCol_BorderShadow
COL_FRAME_BG = ccimgui_dear_bindings.ImGuiCol_FrameBg
COL_FRAME_BG_HOVERED = ccimgui_dear_bindings.ImGuiCol_FrameBgHovered
COL_FRAME_BG_ACTIVE = ccimgui_dear_bindings.ImGuiCol_FrameBgActive
COL_TITLE_BG = ccimgui_dear_bindings.ImGuiCol_TitleBg
COL_TITLE_BG_ACTIVE = ccimgui_dear_bindings.ImGuiCol_TitleBgActive
COL_TITLE_BG_COLLAPSED = ccimgui_dear_bindings.ImGuiCol_TitleBgCollapsed
COL_MENU_BAR_BG = ccimgui_dear_bindings.ImGuiCol_MenuBarBg
COL_SCROLLBAR_BG = ccimgui_dear_bindings.ImGuiCol_ScrollbarBg
COL_SCROLLBAR_GRAB = ccimgui_dear_bindings.ImGuiCol_ScrollbarGrab
COL_SCROLLBAR_GRAB_HOVERED = ccimgui_dear_bindings.ImGuiCol_ScrollbarGrabHovered
COL_SCROLLBAR_GRAB_ACTIVE = ccimgui_dear_bindings.ImGuiCol_ScrollbarGrabActive
COL_CHECK_MARK = ccimgui_dear_bindings.ImGuiCol_CheckMark
COL_SLIDER_GRAB = ccimgui_dear_bindings.ImGuiCol_SliderGrab
COL_SLIDER_GRAB_ACTIVE = ccimgui_dear_bindings.ImGuiCol_SliderGrabActive
COL_BUTTON = ccimgui_dear_bindings.ImGuiCol_Button
COL_BUTTON_HOVERED = ccimgui_dear_bindings.ImGuiCol_ButtonHovered
COL_BUTTON_ACTIVE = ccimgui_dear_bindings.ImGuiCol_ButtonActive
COL_HEADER = ccimgui_dear_bindings.ImGuiCol_Header
COL_HEADER_HOVERED = ccimgui_dear_bindings.ImGuiCol_HeaderHovered
COL_HEADER_ACTIVE = ccimgui_dear_bindings.ImGuiCol_HeaderActive
COL_SEPARATOR = ccimgui_dear_bindings.ImGuiCol_Separator
COL_SEPARATOR_HOVERED = ccimgui_dear_bindings.ImGuiCol_SeparatorHovered
COL_SEPARATOR_ACTIVE = ccimgui_dear_bindings.ImGuiCol_SeparatorActive
COL_RESIZE_GRIP = ccimgui_dear_bindings.ImGuiCol_ResizeGrip
COL_RESIZE_GRIP_HOVERED = ccimgui_dear_bindings.ImGuiCol_ResizeGripHovered
COL_RESIZE_GRIP_ACTIVE = ccimgui_dear_bindings.ImGuiCol_ResizeGripActive
COL_TAB = ccimgui_dear_bindings.ImGuiCol_Tab
COL_TAB_HOVERED = ccimgui_dear_bindings.ImGuiCol_TabHovered
COL_TAB_ACTIVE = ccimgui_dear_bindings.ImGuiCol_TabActive
COL_TAB_UNFOCUSED = ccimgui_dear_bindings.ImGuiCol_TabUnfocused
COL_TAB_UNFOCUSED_ACTIVE = ccimgui_dear_bindings.ImGuiCol_TabUnfocusedActive
COL_DOCKING_PREVIEW = ccimgui_dear_bindings.ImGuiCol_DockingPreview
COL_DOCKING_EMPTY_BG = ccimgui_dear_bindings.ImGuiCol_DockingEmptyBg
COL_PLOT_LINES = ccimgui_dear_bindings.ImGuiCol_PlotLines
COL_PLOT_LINES_HOVERED = ccimgui_dear_bindings.ImGuiCol_PlotLinesHovered
COL_PLOT_HISTOGRAM = ccimgui_dear_bindings.ImGuiCol_PlotHistogram
COL_PLOT_HISTOGRAM_HOVERED = ccimgui_dear_bindings.ImGuiCol_PlotHistogramHovered
COL_TABLE_HEADER_BG = ccimgui_dear_bindings.ImGuiCol_TableHeaderBg
COL_TABLE_BORDER_STRONG = ccimgui_dear_bindings.ImGuiCol_TableBorderStrong
COL_TABLE_BORDER_LIGHT = ccimgui_dear_bindings.ImGuiCol_TableBorderLight
COL_TABLE_ROW_BG = ccimgui_dear_bindings.ImGuiCol_TableRowBg
COL_TABLE_ROW_BG_ALT = ccimgui_dear_bindings.ImGuiCol_TableRowBgAlt
COL_TEXT_SELECTED_BG = ccimgui_dear_bindings.ImGuiCol_TextSelectedBg
COL_DRAG_DROP_TARGET = ccimgui_dear_bindings.ImGuiCol_DragDropTarget
COL_NAV_HIGHLIGHT = ccimgui_dear_bindings.ImGuiCol_NavHighlight
COL_NAV_WINDOWING_HIGHLIGHT = ccimgui_dear_bindings.ImGuiCol_NavWindowingHighlight
COL_NAV_WINDOWING_DIM_BG = ccimgui_dear_bindings.ImGuiCol_NavWindowingDimBg
COL_MODAL_WINDOW_DIM_BG = ccimgui_dear_bindings.ImGuiCol_ModalWindowDimBg
COL_COUNT = ccimgui_dear_bindings.ImGuiCol_COUNT
STYLE_VAR_ALPHA = ccimgui_dear_bindings.ImGuiStyleVar_Alpha
STYLE_VAR_DISABLED_ALPHA = ccimgui_dear_bindings.ImGuiStyleVar_DisabledAlpha
STYLE_VAR_WINDOW_PADDING = ccimgui_dear_bindings.ImGuiStyleVar_WindowPadding
STYLE_VAR_WINDOW_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_WindowRounding
STYLE_VAR_WINDOW_BORDER_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_WindowBorderSize
STYLE_VAR_WINDOW_MIN_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_WindowMinSize
STYLE_VAR_WINDOW_TITLE_ALIGN = ccimgui_dear_bindings.ImGuiStyleVar_WindowTitleAlign
STYLE_VAR_CHILD_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_ChildRounding
STYLE_VAR_CHILD_BORDER_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_ChildBorderSize
STYLE_VAR_POPUP_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_PopupRounding
STYLE_VAR_POPUP_BORDER_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_PopupBorderSize
STYLE_VAR_FRAME_PADDING = ccimgui_dear_bindings.ImGuiStyleVar_FramePadding
STYLE_VAR_FRAME_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_FrameRounding
STYLE_VAR_FRAME_BORDER_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_FrameBorderSize
STYLE_VAR_ITEM_SPACING = ccimgui_dear_bindings.ImGuiStyleVar_ItemSpacing
STYLE_VAR_ITEM_INNER_SPACING = ccimgui_dear_bindings.ImGuiStyleVar_ItemInnerSpacing
STYLE_VAR_INDENT_SPACING = ccimgui_dear_bindings.ImGuiStyleVar_IndentSpacing
STYLE_VAR_CELL_PADDING = ccimgui_dear_bindings.ImGuiStyleVar_CellPadding
STYLE_VAR_SCROLLBAR_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_ScrollbarSize
STYLE_VAR_SCROLLBAR_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_ScrollbarRounding
STYLE_VAR_GRAB_MIN_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_GrabMinSize
STYLE_VAR_GRAB_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_GrabRounding
STYLE_VAR_TAB_ROUNDING = ccimgui_dear_bindings.ImGuiStyleVar_TabRounding
STYLE_VAR_BUTTON_TEXT_ALIGN = ccimgui_dear_bindings.ImGuiStyleVar_ButtonTextAlign
STYLE_VAR_SELECTABLE_TEXT_ALIGN = ccimgui_dear_bindings.ImGuiStyleVar_SelectableTextAlign
STYLE_VAR_SEPARATOR_TEXT_BORDER_SIZE = ccimgui_dear_bindings.ImGuiStyleVar_SeparatorTextBorderSize
STYLE_VAR_SEPARATOR_TEXT_ALIGN = ccimgui_dear_bindings.ImGuiStyleVar_SeparatorTextAlign
STYLE_VAR_SEPARATOR_TEXT_PADDING = ccimgui_dear_bindings.ImGuiStyleVar_SeparatorTextPadding
STYLE_VAR_COUNT = ccimgui_dear_bindings.ImGuiStyleVar_COUNT
BUTTON_FLAGS_NONE = ccimgui_dear_bindings.ImGuiButtonFlags_None
BUTTON_FLAGS_MOUSE_BUTTON_LEFT = ccimgui_dear_bindings.ImGuiButtonFlags_MouseButtonLeft
BUTTON_FLAGS_MOUSE_BUTTON_RIGHT = ccimgui_dear_bindings.ImGuiButtonFlags_MouseButtonRight
BUTTON_FLAGS_MOUSE_BUTTON_MIDDLE = ccimgui_dear_bindings.ImGuiButtonFlags_MouseButtonMiddle
BUTTON_FLAGS_MOUSE_BUTTON_MASK = ccimgui_dear_bindings.ImGuiButtonFlags_MouseButtonMask_
BUTTON_FLAGS_MOUSE_BUTTON_DEFAULT = ccimgui_dear_bindings.ImGuiButtonFlags_MouseButtonDefault_
COLOR_EDIT_FLAGS_NONE = ccimgui_dear_bindings.ImGuiColorEditFlags_None
COLOR_EDIT_FLAGS_NO_ALPHA = ccimgui_dear_bindings.ImGuiColorEditFlags_NoAlpha
COLOR_EDIT_FLAGS_NO_PICKER = ccimgui_dear_bindings.ImGuiColorEditFlags_NoPicker
COLOR_EDIT_FLAGS_NO_OPTIONS = ccimgui_dear_bindings.ImGuiColorEditFlags_NoOptions
COLOR_EDIT_FLAGS_NO_SMALL_PREVIEW = ccimgui_dear_bindings.ImGuiColorEditFlags_NoSmallPreview
COLOR_EDIT_FLAGS_NO_INPUTS = ccimgui_dear_bindings.ImGuiColorEditFlags_NoInputs
COLOR_EDIT_FLAGS_NO_TOOLTIP = ccimgui_dear_bindings.ImGuiColorEditFlags_NoTooltip
COLOR_EDIT_FLAGS_NO_LABEL = ccimgui_dear_bindings.ImGuiColorEditFlags_NoLabel
COLOR_EDIT_FLAGS_NO_SIDE_PREVIEW = ccimgui_dear_bindings.ImGuiColorEditFlags_NoSidePreview
COLOR_EDIT_FLAGS_NO_DRAG_DROP = ccimgui_dear_bindings.ImGuiColorEditFlags_NoDragDrop
COLOR_EDIT_FLAGS_NO_BORDER = ccimgui_dear_bindings.ImGuiColorEditFlags_NoBorder
COLOR_EDIT_FLAGS_ALPHA_BAR = ccimgui_dear_bindings.ImGuiColorEditFlags_AlphaBar
COLOR_EDIT_FLAGS_ALPHA_PREVIEW = ccimgui_dear_bindings.ImGuiColorEditFlags_AlphaPreview
COLOR_EDIT_FLAGS_ALPHA_PREVIEW_HALF = ccimgui_dear_bindings.ImGuiColorEditFlags_AlphaPreviewHalf
COLOR_EDIT_FLAGS_HDR = ccimgui_dear_bindings.ImGuiColorEditFlags_HDR
COLOR_EDIT_FLAGS_DISPLAY_RGB = ccimgui_dear_bindings.ImGuiColorEditFlags_DisplayRGB
COLOR_EDIT_FLAGS_DISPLAY_HSV = ccimgui_dear_bindings.ImGuiColorEditFlags_DisplayHSV
COLOR_EDIT_FLAGS_DISPLAY_HEX = ccimgui_dear_bindings.ImGuiColorEditFlags_DisplayHex
COLOR_EDIT_FLAGS_UINT8 = ccimgui_dear_bindings.ImGuiColorEditFlags_Uint8
COLOR_EDIT_FLAGS_FLOAT = ccimgui_dear_bindings.ImGuiColorEditFlags_Float
COLOR_EDIT_FLAGS_PICKER_HUE_BAR = ccimgui_dear_bindings.ImGuiColorEditFlags_PickerHueBar
COLOR_EDIT_FLAGS_PICKER_HUE_WHEEL = ccimgui_dear_bindings.ImGuiColorEditFlags_PickerHueWheel
COLOR_EDIT_FLAGS_INPUT_RGB = ccimgui_dear_bindings.ImGuiColorEditFlags_InputRGB
COLOR_EDIT_FLAGS_INPUT_HSV = ccimgui_dear_bindings.ImGuiColorEditFlags_InputHSV
COLOR_EDIT_FLAGS_DEFAULT_OPTIONS = ccimgui_dear_bindings.ImGuiColorEditFlags_DefaultOptions_
COLOR_EDIT_FLAGS_DISPLAY_MASK = ccimgui_dear_bindings.ImGuiColorEditFlags_DisplayMask_
COLOR_EDIT_FLAGS_DATA_TYPE_MASK = ccimgui_dear_bindings.ImGuiColorEditFlags_DataTypeMask_
COLOR_EDIT_FLAGS_PICKER_MASK = ccimgui_dear_bindings.ImGuiColorEditFlags_PickerMask_
COLOR_EDIT_FLAGS_INPUT_MASK = ccimgui_dear_bindings.ImGuiColorEditFlags_InputMask_
SLIDER_FLAGS_NONE = ccimgui_dear_bindings.ImGuiSliderFlags_None
SLIDER_FLAGS_ALWAYS_CLAMP = ccimgui_dear_bindings.ImGuiSliderFlags_AlwaysClamp
SLIDER_FLAGS_LOGARITHMIC = ccimgui_dear_bindings.ImGuiSliderFlags_Logarithmic
SLIDER_FLAGS_NO_ROUND_TO_FORMAT = ccimgui_dear_bindings.ImGuiSliderFlags_NoRoundToFormat
SLIDER_FLAGS_NO_INPUT = ccimgui_dear_bindings.ImGuiSliderFlags_NoInput
SLIDER_FLAGS_INVALID_MASK = ccimgui_dear_bindings.ImGuiSliderFlags_InvalidMask_
MOUSE_BUTTON_LEFT = ccimgui_dear_bindings.ImGuiMouseButton_Left
MOUSE_BUTTON_RIGHT = ccimgui_dear_bindings.ImGuiMouseButton_Right
MOUSE_BUTTON_MIDDLE = ccimgui_dear_bindings.ImGuiMouseButton_Middle
MOUSE_BUTTON_COUNT = ccimgui_dear_bindings.ImGuiMouseButton_COUNT
MOUSE_CURSOR_NONE = ccimgui_dear_bindings.ImGuiMouseCursor_None
MOUSE_CURSOR_ARROW = ccimgui_dear_bindings.ImGuiMouseCursor_Arrow
MOUSE_CURSOR_TEXT_INPUT = ccimgui_dear_bindings.ImGuiMouseCursor_TextInput
MOUSE_CURSOR_RESIZE_ALL = ccimgui_dear_bindings.ImGuiMouseCursor_ResizeAll
MOUSE_CURSOR_RESIZE_NS = ccimgui_dear_bindings.ImGuiMouseCursor_ResizeNS
MOUSE_CURSOR_RESIZE_EW = ccimgui_dear_bindings.ImGuiMouseCursor_ResizeEW
MOUSE_CURSOR_RESIZE_NESW = ccimgui_dear_bindings.ImGuiMouseCursor_ResizeNESW
MOUSE_CURSOR_RESIZE_NWSE = ccimgui_dear_bindings.ImGuiMouseCursor_ResizeNWSE
MOUSE_CURSOR_HAND = ccimgui_dear_bindings.ImGuiMouseCursor_Hand
MOUSE_CURSOR_NOT_ALLOWED = ccimgui_dear_bindings.ImGuiMouseCursor_NotAllowed
MOUSE_CURSOR_COUNT = ccimgui_dear_bindings.ImGuiMouseCursor_COUNT
COND_NONE = ccimgui_dear_bindings.ImGuiCond_None
COND_ALWAYS = ccimgui_dear_bindings.ImGuiCond_Always
COND_ONCE = ccimgui_dear_bindings.ImGuiCond_Once
COND_FIRST_USE_EVER = ccimgui_dear_bindings.ImGuiCond_FirstUseEver
COND_APPEARING = ccimgui_dear_bindings.ImGuiCond_Appearing
IM_DRAW_FLAGS_NONE = ccimgui_dear_bindings.ImDrawFlags_None
IM_DRAW_FLAGS_CLOSED = ccimgui_dear_bindings.ImDrawFlags_Closed
IM_DRAW_FLAGS_ROUND_CORNERS_TOP_LEFT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersTopLeft
IM_DRAW_FLAGS_ROUND_CORNERS_TOP_RIGHT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersTopRight
IM_DRAW_FLAGS_ROUND_CORNERS_BOTTOM_LEFT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersBottomLeft
IM_DRAW_FLAGS_ROUND_CORNERS_BOTTOM_RIGHT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersBottomRight
IM_DRAW_FLAGS_ROUND_CORNERS_NONE = ccimgui_dear_bindings.ImDrawFlags_RoundCornersNone
IM_DRAW_FLAGS_ROUND_CORNERS_TOP = ccimgui_dear_bindings.ImDrawFlags_RoundCornersTop
IM_DRAW_FLAGS_ROUND_CORNERS_BOTTOM = ccimgui_dear_bindings.ImDrawFlags_RoundCornersBottom
IM_DRAW_FLAGS_ROUND_CORNERS_LEFT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersLeft
IM_DRAW_FLAGS_ROUND_CORNERS_RIGHT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersRight
IM_DRAW_FLAGS_ROUND_CORNERS_ALL = ccimgui_dear_bindings.ImDrawFlags_RoundCornersAll
IM_DRAW_FLAGS_ROUND_CORNERS_DEFAULT = ccimgui_dear_bindings.ImDrawFlags_RoundCornersDefault_
IM_DRAW_FLAGS_ROUND_CORNERS_MASK = ccimgui_dear_bindings.ImDrawFlags_RoundCornersMask_
IM_DRAW_LIST_FLAGS_NONE = ccimgui_dear_bindings.ImDrawListFlags_None
IM_DRAW_LIST_FLAGS_ANTI_ALIASED_LINES = ccimgui_dear_bindings.ImDrawListFlags_AntiAliasedLines
IM_DRAW_LIST_FLAGS_ANTI_ALIASED_LINES_USE_TEX = ccimgui_dear_bindings.ImDrawListFlags_AntiAliasedLinesUseTex
IM_DRAW_LIST_FLAGS_ANTI_ALIASED_FILL = ccimgui_dear_bindings.ImDrawListFlags_AntiAliasedFill
IM_DRAW_LIST_FLAGS_ALLOW_VTX_OFFSET = ccimgui_dear_bindings.ImDrawListFlags_AllowVtxOffset
IM_FONT_ATLAS_FLAGS_NONE = ccimgui_dear_bindings.ImFontAtlasFlags_None
IM_FONT_ATLAS_FLAGS_NO_POWER_OF_TWO_HEIGHT = ccimgui_dear_bindings.ImFontAtlasFlags_NoPowerOfTwoHeight
IM_FONT_ATLAS_FLAGS_NO_MOUSE_CURSORS = ccimgui_dear_bindings.ImFontAtlasFlags_NoMouseCursors
IM_FONT_ATLAS_FLAGS_NO_BAKED_LINES = ccimgui_dear_bindings.ImFontAtlasFlags_NoBakedLines
VIEWPORT_FLAGS_NONE = ccimgui_dear_bindings.ImGuiViewportFlags_None
VIEWPORT_FLAGS_IS_PLATFORM_WINDOW = ccimgui_dear_bindings.ImGuiViewportFlags_IsPlatformWindow
VIEWPORT_FLAGS_IS_PLATFORM_MONITOR = ccimgui_dear_bindings.ImGuiViewportFlags_IsPlatformMonitor
VIEWPORT_FLAGS_OWNED_BY_APP = ccimgui_dear_bindings.ImGuiViewportFlags_OwnedByApp
VIEWPORT_FLAGS_NO_DECORATION = ccimgui_dear_bindings.ImGuiViewportFlags_NoDecoration
VIEWPORT_FLAGS_NO_TASK_BAR_ICON = ccimgui_dear_bindings.ImGuiViewportFlags_NoTaskBarIcon
VIEWPORT_FLAGS_NO_FOCUS_ON_APPEARING = ccimgui_dear_bindings.ImGuiViewportFlags_NoFocusOnAppearing
VIEWPORT_FLAGS_NO_FOCUS_ON_CLICK = ccimgui_dear_bindings.ImGuiViewportFlags_NoFocusOnClick
VIEWPORT_FLAGS_NO_INPUTS = ccimgui_dear_bindings.ImGuiViewportFlags_NoInputs
VIEWPORT_FLAGS_NO_RENDERER_CLEAR = ccimgui_dear_bindings.ImGuiViewportFlags_NoRendererClear
VIEWPORT_FLAGS_TOP_MOST = ccimgui_dear_bindings.ImGuiViewportFlags_TopMost
VIEWPORT_FLAGS_MINIMIZED = ccimgui_dear_bindings.ImGuiViewportFlags_Minimized
VIEWPORT_FLAGS_NO_AUTO_MERGE = ccimgui_dear_bindings.ImGuiViewportFlags_NoAutoMerge
VIEWPORT_FLAGS_CAN_HOST_OTHER_WINDOWS = ccimgui_dear_bindings.ImGuiViewportFlags_CanHostOtherWindows
# [End Enums]
...
def get_key_index(key: int):
    cdef ccimgui_dear_bindings.ImGuiKey res = ccimgui_dear_bindings.GetKeyIndex(
        key
    )
    return res
# [End Function]
...
def begin_child_id(id_: int, size: tuple=(0, 0), border: bool=False, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_BeginChildID(
        id_,
        _cast_tuple_ImVec2(size),
        border,
        flags
...
def begin_table_ex(str_id: str, column: int, flags: int=0, outer_size: tuple=(0.0, 0.0), inner_width: float=0.0):
    cdef bool res = ccimgui_dear_bindings.ImGui_BeginTableEx(
        _bytes(str_id),
        column,
        flags,
...
def bullet_text_v(fmt: str):
    ccimgui_dear_bindings.ImGui_BulletTextV(
        _bytes(fmt)
    )
# [End Function]
...
def calc_text_size_ex(text: str, text_end: str=None, hide_text_after_double_hash: bool=False, wrap_width: float=-1.0):
    cdef ccimgui_dear_bindings.ImVec2 res = ccimgui_dear_bindings.ImGui_CalcTextSizeEx(
        _bytes(text),
        _bytes(text_end),
...
def checkbox(label: str, v: BoolPtr):
    cdef bool res = ccimgui_dear_bindings.ImGui_Checkbox(
        _bytes(label),
        &v.value
    )
    return res
...
def checkbox_flags_int_ptr(label: str, flags: IntPtr, flags_value: int):
    cdef bool res = ccimgui_dear_bindings.ImGui_CheckboxFlagsIntPtr(
        _bytes(label),
        &flags.value,
        flags_value
    )
...
def checkbox_flags_uint_ptr(label: str, flags: IntPtr, flags_value: int):
    cdef bool res = ccimgui_dear_bindings.ImGui_CheckboxFlagsUintPtr(
        _bytes(label),
        flags,
        flags_value
    )
...
def color_convert_float4_to_u32(in_: tuple):
    cdef ccimgui_dear_bindings.ImU32 res = ccimgui_dear_bindings.ImGui_ColorConvertFloat4ToU32(
...
def color_convert_hsv_to_rgb(h: float, s: float, v: float, out_r: FloatPtr, out_g: FloatPtr, out_b: FloatPtr):
    ccimgui_dear_bindings.ImGui_ColorConvertHSVtoRGB(
        h,
        s,
        v,
        &out_r.value,
...
def color_convert_rgb_to_hsv(r: float, g: float, b: float, out_h: FloatPtr, out_s: FloatPtr, out_v: FloatPtr):
    ccimgui_dear_bindings.ImGui_ColorConvertRGBtoHSV(
        r,
        g,
        b,
        &out_h.value,
...
def color_edit4(label: str, col: Sequence[FloatPtr], flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_ColorEdit4(
        _bytes(label),
        &col.value,
        flags
    )
...
def color_picker3(label: str, col: Sequence[FloatPtr], flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_ColorPicker3(
        _bytes(label),
        &col.value,
        flags
    )
...
def color_picker4(label: str, col: Sequence[FloatPtr], flags: int=0, ref_col: FloatPtr=None):
    cdef bool res = ccimgui_dear_bindings.ImGui_ColorPicker4(
        _bytes(label),
        &col.value,
        flags,
        &ref_col.value
...
def columns_ex(count: int=1, id_: str=None, border: bool=True):
    ccimgui_dear_bindings.ImGui_ColumnsEx(
        count,
        _bytes(id_),
        border
    )
...
def combo_callback_ex(label: str, current_item: IntPtr, items_getter: Callable, data: Any, items_count: int, popup_max_height_in_items: int=-1):
    cdef bool res = ccimgui_dear_bindings.ImGui_ComboCallbackEx(
        _bytes(label),
        &current_item.value,
        items_getter,
        data,
...
def combo_char_ex(label: str, current_item: IntPtr, items: str, items_count: int, popup_max_height_in_items: int=-1):
    cdef bool res = ccimgui_dear_bindings.ImGui_ComboCharEx(
        _bytes(label),
        &current_item.value,
        _bytes(items),
        items_count,
...
def dock_space_ex(id_: int, size: tuple=(0, 0), flags: int=0, window_class: ImGuiWindowClass=None):
    cdef ccimgui_dear_bindings.ImGuiID res = ccimgui_dear_bindings.ImGui_DockSpaceEx(
        id_,
        _cast_tuple_ImVec2(size),
        flags,
        window_class
...
def dock_space_over_viewport_ex(viewport: ImGuiViewport=None, flags: int=0, window_class: ImGuiWindowClass=None):
    cdef ccimgui_dear_bindings.ImGuiID res = ccimgui_dear_bindings.ImGui_DockSpaceOverViewportEx(
        viewport,
        flags,
        window_class
    )
...
def drag_float2_ex(label: str, v: Sequence[FloatPtr], v_speed: float=1.0, v_min: float=0.0, v_max: float=0.0, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragFloat2Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_float3_ex(label: str, v: Sequence[FloatPtr], v_speed: float=1.0, v_min: float=0.0, v_max: float=0.0, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragFloat3Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_float4_ex(label: str, v: Sequence[FloatPtr], v_speed: float=1.0, v_min: float=0.0, v_max: float=0.0, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragFloat4Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_float_range2_ex(label: str, v_current_min: FloatPtr, v_current_max: FloatPtr, v_speed: float=1.0, v_min: float=0.0, v_max: float=0.0, format_: str="%.3f", format_max: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragFloatRange2Ex(
        _bytes(label),
        &v_current_min.value,
        &v_current_max.value,
        v_speed,
...
def drag_int2_ex(label: str, v: Sequence[IntPtr], v_speed: float=1.0, v_min: int=0, v_max: int=0, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragInt2Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_int3_ex(label: str, v: Sequence[IntPtr], v_speed: float=1.0, v_min: int=0, v_max: int=0, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragInt3Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_int4_ex(label: str, v: Sequence[IntPtr], v_speed: float=1.0, v_min: int=0, v_max: int=0, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragInt4Ex(
        _bytes(label),
        &v.value,
        v_speed,
        v_min,
...
def drag_int_range2_ex(label: str, v_current_min: IntPtr, v_current_max: IntPtr, v_speed: float=1.0, v_min: int=0, v_max: int=0, format_: str="%d", format_max: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragIntRange2Ex(
        _bytes(label),
        &v_current_min.value,
        &v_current_max.value,
        v_speed,
...
def drag_scalar_ex(label: str, data_type: int, p_data: Any, v_speed: float=1.0, p_min: Any=None, p_max: Any=None, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragScalarEx(
        _bytes(label),
        data_type,
        p_data,
        v_speed,
...
def drag_scalar_ne_x(label: str, data_type: int, p_data: Any, components: int, v_speed: float=1.0, p_min: Any=None, p_max: Any=None, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_DragScalarNEx(
        _bytes(label),
        data_type,
        p_data,
        components,
...
# ?returns(None)
def end():
    ccimgui_dear_bindings.ImGui_End()
# [End Function]
...
# ?returns(None)
def end_child():
    ccimgui_dear_bindings.ImGui_EndChild()
# [End Function]
...
# ?returns(None)
def end_disabled():
    ccimgui_dear_bindings.ImGui_EndDisabled()
# [End Function]
...
def get_allocator_functions(p_alloc_func: Callable, p_free_func: Callable, p_user_data: Any):
    ccimgui_dear_bindings.ImGui_GetAllocatorFunctions(
        p_alloc_func,
        p_free_func,
        p_user_data
    )
...
def get_columns_count():
    cdef int res = ccimgui_dear_bindings.ImGui_GetColumnsCount()
    return res
# [End Function]
...
def get_current_context():
    cdef ccimgui_dear_bindings.ImGuiContext* res = ccimgui_dear_bindings.ImGui_GetCurrentContext()
    return ImGuiContext.from_ptr(res)
# [End Function]
...
def get_idp_tr(ptr_id: Any):
    cdef ccimgui_dear_bindings.ImGuiID res = ccimgui_dear_bindings.ImGui_GetIDPtr(
        ptr_id
    )
    return res
# [End Function]
...
def get_ids_tr(str_id_begin: str, str_id_end: str):
    cdef ccimgui_dear_bindings.ImGuiID res = ccimgui_dear_bindings.ImGui_GetIDStr(
        _bytes(str_id_begin),
        _bytes(str_id_end)
    )
    return res
...
def get_state_storage():
    cdef ccimgui_dear_bindings.ImGuiStorage* res = ccimgui_dear_bindings.ImGui_GetStateStorage()
...
def get_window_dock_id():
    cdef ccimgui_dear_bindings.ImGuiID res = ccimgui_dear_bindings.ImGui_GetWindowDockID()
    return res
# [End Function]
...
def image_button_ex(str_id: str, user_texture_id: Any, size: tuple, uv0: tuple=(0, 0), uv1: tuple=(1, 1), bg_col: tuple=(0, 0, 0, 0), tint_col: tuple=(1, 1, 1, 1)):
    cdef bool res = ccimgui_dear_bindings.ImGui_ImageButtonEx(
        _bytes(str_id),
        user_texture_id,
        _cast_tuple_ImVec2(size),
...
def image_ex(user_texture_id: Any, size: tuple, uv0: tuple=(0, 0), uv1: tuple=(1, 1), tint_col: tuple=(1, 1, 1, 1), border_col: tuple=(0, 0, 0, 0)):
    ccimgui_dear_bindings.ImGui_ImageEx(
        user_texture_id,
        _cast_tuple_ImVec2(size),
        _cast_tuple_ImVec2(uv0),
        _cast_tuple_ImVec2(uv1),
...
def input_double_ex(label: str, v: DoublePtr, step: float=0.0, step_fast: float=0.0, format_: str="%.6f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputDoubleEx(
        _bytes(label),
        &v.value,
        step,
        step_fast,
...
def input_float2_ex(label: str, v: Sequence[FloatPtr], format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputFloat2Ex(
        _bytes(label),
        &v.value,
        _bytes(format_),
        flags
...
def input_float3_ex(label: str, v: Sequence[FloatPtr], format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputFloat3Ex(
        _bytes(label),
        &v.value,
        _bytes(format_),
        flags
...
def input_float4_ex(label: str, v: Sequence[FloatPtr], format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputFloat4Ex(
        _bytes(label),
        &v.value,
        _bytes(format_),
        flags
...
def input_float_ex(label: str, v: FloatPtr, step: float=0.0, step_fast: float=0.0, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputFloatEx(
        _bytes(label),
        &v.value,
        step,
        step_fast,
...
def input_int2(label: str, v: Sequence[IntPtr], flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputInt2(
        _bytes(label),
        &v.value,
        flags
    )
...
def input_int3(label: str, v: Sequence[IntPtr], flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputInt3(
        _bytes(label),
        &v.value,
        flags
    )
...
def input_int4(label: str, v: Sequence[IntPtr], flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputInt4(
        _bytes(label),
        &v.value,
        flags
    )
...
def input_int_ex(label: str, v: IntPtr, step: int=1, step_fast: int=100, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputIntEx(
        _bytes(label),
        &v.value,
        step,
        step_fast,
...
def input_scalar_ex(label: str, data_type: int, p_data: Any, p_step: Any=None, p_step_fast: Any=None, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputScalarEx(
        _bytes(label),
        data_type,
        p_data,
        p_step,
...
def input_scalar_ne_x(label: str, data_type: int, p_data: Any, components: int, p_step: Any=None, p_step_fast: Any=None, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputScalarNEx(
        _bytes(label),
        data_type,
        p_data,
        components,
...
def input_text_ex(label: str, buf: str, buf_size: Any, flags: int=0, callback: Callable=None, user_data: Any=None):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputTextEx(
        _bytes(label),
        _bytes(buf),
        buf_size,
        flags,
...
def input_text_multiline_ex(label: str, buf: str, buf_size: Any, size: tuple=(0, 0), flags: int=0, callback: Callable=None, user_data: Any=None):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputTextMultilineEx(
        _bytes(label),
        _bytes(buf),
        buf_size,
        _cast_tuple_ImVec2(size),
...
def input_text_with_hint_ex(label: str, hint: str, buf: str, buf_size: Any, flags: int=0, callback: Callable=None, user_data: Any=None):
    cdef bool res = ccimgui_dear_bindings.ImGui_InputTextWithHintEx(
        _bytes(label),
        _bytes(hint),
        _bytes(buf),
        buf_size,
...
def is_window_collapsed():
    cdef bool res = ccimgui_dear_bindings.ImGui_IsWindowCollapsed()
    return res
# [End Function]
...
def label_text_v(label: str, fmt: str):
    ccimgui_dear_bindings.ImGui_LabelTextV(
        _bytes(label),
        _bytes(fmt)
    )
# [End Function]
...
def list_box(label: str, current_item: IntPtr, items: str, items_count: int, height_in_items: int=-1):
    cdef bool res = ccimgui_dear_bindings.ImGui_ListBox(
        _bytes(label),
        &current_item.value,
        _bytes(items),
        items_count,
...
def list_box_callback_ex(label: str, current_item: IntPtr, items_getter: Callable, data: Any, items_count: int, height_in_items: int=-1):
    cdef bool res = ccimgui_dear_bindings.ImGui_ListBoxCallbackEx(
        _bytes(label),
        &current_item.value,
        items_getter,
        data,
...
def log_text_v(fmt: str):
    ccimgui_dear_bindings.ImGui_LogTextV(
        _bytes(fmt)
    )
# [End Function]
...
def mem_alloc(size: Any):
    cdef void* res = ccimgui_dear_bindings.ImGui_MemAlloc(
        size
    )
    return res
# [End Function]
...
def mem_free(ptr: Any):
    ccimgui_dear_bindings.ImGui_MemFree(
        ptr
    )
# [End Function]
...
def plot_histogram_callback_ex(label: str, values_getter: Callable, data: Any, values_count: int, values_offset: int=0, overlay_text: str=None, scale_min: float=FLT_MAX, scale_max: float=FLT_MAX, graph_size: tuple=(0, 0)):
    ccimgui_dear_bindings.ImGui_PlotHistogramCallbackEx(
        _bytes(label),
        values_getter,
        data,
        values_count,
...
def plot_histogram_ex(label: str, values: FloatPtr, values_count: int, values_offset: int=0, overlay_text: str=None, scale_min: float=FLT_MAX, scale_max: float=FLT_MAX, graph_size: tuple=(0, 0), stride: int=sizeof(float)):
    ccimgui_dear_bindings.ImGui_PlotHistogramEx(
        _bytes(label),
        &values.value,
        values_count,
        values_offset,
...
def plot_lines_callback_ex(label: str, values_getter: Callable, data: Any, values_count: int, values_offset: int=0, overlay_text: str=None, scale_min: float=FLT_MAX, scale_max: float=FLT_MAX, graph_size: tuple=(0, 0)):
    ccimgui_dear_bindings.ImGui_PlotLinesCallbackEx(
        _bytes(label),
        values_getter,
        data,
        values_count,
...
def plot_lines_ex(label: str, values: FloatPtr, values_count: int, values_offset: int=0, overlay_text: str=None, scale_min: float=FLT_MAX, scale_max: float=FLT_MAX, graph_size: tuple=(0, 0), stride: int=sizeof(float)):
    ccimgui_dear_bindings.ImGui_PlotLinesEx(
        _bytes(label),
        &values.value,
        values_count,
        values_offset,
...
def pop_button_repeat():
    ccimgui_dear_bindings.ImGui_PopButtonRepeat()
# [End Function]
...
# ?returns(None)
def pop_clip_rect():
    ccimgui_dear_bindings.ImGui_PopClipRect()
# [End Function]
...
# ?returns(None)
def pop_font():
    ccimgui_dear_bindings.ImGui_PopFont()
# [End Function]
...
# ?returns(None)
def pop_item_width():
    ccimgui_dear_bindings.ImGui_PopItemWidth()
# [End Function]
...
def pop_style_color_ex(count: int=1):
    ccimgui_dear_bindings.ImGui_PopStyleColorEx(
        count
    )
# [End Function]
...
def pop_style_var_ex(count: int=1):
    ccimgui_dear_bindings.ImGui_PopStyleVarEx(
        count
    )
# [End Function]
...
# ?returns(None)
def pop_tab_stop():
    ccimgui_dear_bindings.ImGui_PopTabStop()
# [End Function]
...
def pop_text_wrap_pos():
    ccimgui_dear_bindings.ImGui_PopTextWrapPos()
# [End Function]
...
def progress_bar(fraction: float, size_arg: tuple=(-FLT_MIN, 0), overlay: str=None):
    ccimgui_dear_bindings.ImGui_ProgressBar(
        fraction,
...
def push_style_color_im_vec4(idx: int, col: tuple):
    ccimgui_dear_bindings.ImGui_PushStyleColorImVec4(
        idx,
        _cast_tuple_ImVec4(col)
    )
# [End Function]
...
def set_clipboard_text(text: str):
    ccimgui_dear_bindings.ImGui_SetClipboardText(
        _bytes(text)
    )
# [End Function]
...
def set_current_context(ctx: ImGuiContext):
    ccimgui_dear_bindings.ImGui_SetCurrentContext(
        ctx
    )
# [End Function]
...
def set_tooltip_v(fmt: str):
    ccimgui_dear_bindings.ImGui_SetTooltipV(
        _bytes(fmt)
    )
# [End Function]
...
def slider_angle_ex(label: str, v_rad: FloatPtr, v_degrees_min: float=-360.0, v_degrees_max: float=+360.0, format_: str="%.0f deg", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderAngleEx(
        _bytes(label),
        &v_rad.value,
        v_degrees_min,
        v_degrees_max,
...
def slider_float2_ex(label: str, v: Sequence[FloatPtr], v_min: float, v_max: float, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderFloat2Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_float3_ex(label: str, v: Sequence[FloatPtr], v_min: float, v_max: float, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderFloat3Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_float4_ex(label: str, v: Sequence[FloatPtr], v_min: float, v_max: float, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderFloat4Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_int2_ex(label: str, v: Sequence[IntPtr], v_min: int, v_max: int, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderInt2Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_int3_ex(label: str, v: Sequence[IntPtr], v_min: int, v_max: int, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderInt3Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_int4_ex(label: str, v: Sequence[IntPtr], v_min: int, v_max: int, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderInt4Ex(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_int_ex(label: str, v: IntPtr, v_min: int, v_max: int, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderIntEx(
        _bytes(label),
        &v.value,
        v_min,
        v_max,
...
def slider_scalar_ex(label: str, data_type: int, p_data: Any, p_min: Any, p_max: Any, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderScalarEx(
        _bytes(label),
        data_type,
        p_data,
        p_min,
...
def slider_scalar_ne_x(label: str, data_type: int, p_data: Any, components: int, p_min: Any, p_max: Any, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_SliderScalarNEx(
        _bytes(label),
        data_type,
        p_data,
        components,
...
def table_setup_column_ex(label: str, flags: int=0, init_width_or_weight: float=0.0, user_id: int=0):
    ccimgui_dear_bindings.ImGui_TableSetupColumnEx(
        _bytes(label),
        flags,
...
def text_colored_v(col: tuple, fmt: str):
    ccimgui_dear_bindings.ImGui_TextColoredV(
        _cast_tuple_ImVec4(col),
        _bytes(fmt)
    )
# [End Function]
...
def text_disabled_v(fmt: str):
    ccimgui_dear_bindings.ImGui_TextDisabledV(
        _bytes(fmt)
    )
# [End Function]
...
# ?returns(None)
def text_v(fmt: str):
    ccimgui_dear_bindings.ImGui_TextV(
        _bytes(fmt)
    )
# [End Function]
...
def text_wrapped_v(fmt: str):
    ccimgui_dear_bindings.ImGui_TextWrappedV(
        _bytes(fmt)
    )
# [End Function]
...
def tree_node_ex(label: str, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeEx(
        _bytes(label),
        flags
    )
    return res
...
def tree_node_ex_ptr(ptr_id: Any, flags: int, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeExPtr(
        ptr_id,
        flags,
        _bytes(fmt)
    )
...
def tree_node_ex_str(str_id: str, flags: int, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeExStr(
        _bytes(str_id),
        flags,
        _bytes(fmt)
    )
...
def tree_node_ex_v(str_id: str, flags: int, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeExV(
        _bytes(str_id),
        flags,
        _bytes(fmt)
    )
...
def tree_node_ex_vp_tr(ptr_id: Any, flags: int, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeExVPtr(
        ptr_id,
        flags,
        _bytes(fmt)
    )
...
def tree_node_v(str_id: str, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeV(
        _bytes(str_id),
        _bytes(fmt)
    )
    return res
...
def tree_node_vp_tr(ptr_id: Any, fmt: str):
    cdef bool res = ccimgui_dear_bindings.ImGui_TreeNodeVPtr(
        ptr_id,
        _bytes(fmt)
    )
    return res
...
# ?active(False)
# ?returns(bool)
def vs_lider_float(label: str, size: tuple, v: FloatPtr, v_min: float, v_max: float):
    """
    Implied format = '%.3f', flags = 0
    """
...
# ?active(False)
# ?returns(bool)
def vs_lider_float_ex(label: str, size: tuple, v: FloatPtr, v_min: float, v_max: float, format_: str="%.3f", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_VSliderFloatEx(
        _bytes(label),
        _cast_tuple_ImVec2(size),
        &v.value,
        v_min,
...
# ?active(False)
# ?returns(bool)
def vs_lider_int(label: str, size: tuple, v: IntPtr, v_min: int, v_max: int):
    """
    Implied format = '%d', flags = 0
    """
...
# ?active(False)
# ?returns(bool)
def vs_lider_int_ex(label: str, size: tuple, v: IntPtr, v_min: int, v_max: int, format_: str="%d", flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_VSliderIntEx(
        _bytes(label),
        _cast_tuple_ImVec2(size),
        &v.value,
        v_min,
...
# ?active(False)
# ?returns(bool)
def vs_lider_scalar(label: str, size: tuple, data_type: int, p_data: Any, p_min: Any, p_max: Any):
    """
    Implied format = null, flags = 0
    """
...
# ?active(False)
# ?returns(bool)
def vs_lider_scalar_ex(label: str, size: tuple, data_type: int, p_data: Any, p_min: Any, p_max: Any, format_: str=None, flags: int=0):
    cdef bool res = ccimgui_dear_bindings.ImGui_VSliderScalarEx(
        _bytes(label),
        _cast_tuple_ImVec2(size),
        data_type,
        p_data,
...
cdef class ImDrawListSharedData:
    ...
    @staticmethod
    cdef ImDrawListSharedData from_ptr(ccimgui_dear_bindings.ImDrawListSharedData* _ptr):
...
cdef class ImFontBuilderIO:
    ...
    @staticmethod
    cdef ImFontBuilderIO from_ptr(ccimgui_dear_bindings.ImFontBuilderIO* _ptr):
...
cdef class ImGuiContext:
    ...
    @staticmethod
    cdef ImGuiContext from_ptr(ccimgui_dear_bindings.ImGuiContext* _ptr):
...
# ?use_template(False)
cdef class ImVec2:
    cdef ccimgui_dear_bindings.ImVec2* _ptr
    
    @staticmethod
    cdef ImVec2 from_ptr(ccimgui_dear_bindings.ImVec2* _ptr):
...
cdef class ImVec4:
    ...
    @staticmethod
    cdef ImVec4 from_ptr(ccimgui_dear_bindings.ImVec4* _ptr):
...
cdef class ImVector_ImWchar:
    cdef ccimgui_dear_bindings.ImVector_ImWchar* _ptr
    
    @staticmethod
    cdef ImVector_ImWchar from_ptr(ccimgui_dear_bindings.ImVector_ImWchar* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImWchar* res = dereference(self._ptr).Data
        return res
    @data.setter
...
cdef class ImVector_ImGuiTextFilter_ImGuiTextRange:
    cdef ccimgui_dear_bindings.ImVector_ImGuiTextFilter_ImGuiTextRange* _ptr
    
    @staticmethod
    cdef ImVector_ImGuiTextFilter_ImGuiTextRange from_ptr(ccimgui_dear_bindings.ImVector_ImGuiTextFilter_ImGuiTextRange* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImGuiTextFilter_ImGuiTextRange* res = dereference(self._ptr).Data
...
cdef class ImVector_char:
    cdef ccimgui_dear_bindings.ImVector_char* _ptr
    
    @staticmethod
    cdef ImVector_char from_ptr(ccimgui_dear_bindings.ImVector_char* _ptr):
...
cdef class ImVector_ImGuiStorage_ImGuiStoragePair:
    cdef ccimgui_dear_bindings.ImVector_ImGuiStorage_ImGuiStoragePair* _ptr
    
    @staticmethod
    cdef ImVector_ImGuiStorage_ImGuiStoragePair from_ptr(ccimgui_dear_bindings.ImVector_ImGuiStorage_ImGuiStoragePair* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImGuiStorage_ImGuiStoragePair* res = dereference(self._ptr).Data
...
cdef class ImVector_ImDrawCmd:
    cdef ccimgui_dear_bindings.ImVector_ImDrawCmd* _ptr
    
    @staticmethod
    cdef ImVector_ImDrawCmd from_ptr(ccimgui_dear_bindings.ImVector_ImDrawCmd* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImDrawCmd* res = dereference(self._ptr).Data
...
cdef class ImVector_ImDrawIdx:
    cdef ccimgui_dear_bindings.ImVector_ImDrawIdx* _ptr
    
    @staticmethod
    cdef ImVector_ImDrawIdx from_ptr(ccimgui_dear_bindings.ImVector_ImDrawIdx* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImDrawIdx* res = dereference(self._ptr).Data
        return res
    @data.setter
    def data(self, value: int):
        dereference(self._ptr).Data = value
...
cdef class ImVector_ImDrawChannel:
    cdef ccimgui_dear_bindings.ImVector_ImDrawChannel* _ptr
    
    @staticmethod
    cdef ImVector_ImDrawChannel from_ptr(ccimgui_dear_bindings.ImVector_ImDrawChannel* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImDrawChannel* res = dereference(self._ptr).Data
...
cdef class ImVector_ImDrawVert:
    cdef ccimgui_dear_bindings.ImVector_ImDrawVert* _ptr
    
    @staticmethod
    cdef ImVector_ImDrawVert from_ptr(ccimgui_dear_bindings.ImVector_ImDrawVert* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImDrawVert* res = dereference(self._ptr).Data
...
cdef class ImVector_ImVec4:
    cdef ccimgui_dear_bindings.ImVector_ImVec4* _ptr
    
    @staticmethod
    cdef ImVector_ImVec4 from_ptr(ccimgui_dear_bindings.ImVector_ImVec4* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImVec4* res = dereference(self._ptr).Data
        return ImVec4.from_ptr(res)
    @data.setter
...
cdef class ImVector_ImTextureID:
    cdef ccimgui_dear_bindings.ImVector_ImTextureID* _ptr
    
    @staticmethod
    cdef ImVector_ImTextureID from_ptr(ccimgui_dear_bindings.ImVector_ImTextureID* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImTextureID* res = dereference(self._ptr).Data
        return res
    @data.setter
...
cdef class ImVector_ImVec2:
    cdef ccimgui_dear_bindings.ImVector_ImVec2* _ptr
    
    @staticmethod
    cdef ImVector_ImVec2 from_ptr(ccimgui_dear_bindings.ImVector_ImVec2* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImVec2* res = dereference(self._ptr).Data
...
cdef class ImVector_ImU32:
    cdef ccimgui_dear_bindings.ImVector_ImU32* _ptr
    
    @staticmethod
    cdef ImVector_ImU32 from_ptr(ccimgui_dear_bindings.ImVector_ImU32* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImU32* res = dereference(self._ptr).Data
        return res
    @data.setter
    def data(self, value: int):
        dereference(self._ptr).Data = value
...
cdef class ImVector_ImFontPtr:
    cdef ccimgui_dear_bindings.ImVector_ImFontPtr* _ptr
    
    @staticmethod
    cdef ImVector_ImFontPtr from_ptr(ccimgui_dear_bindings.ImVector_ImFontPtr* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImFont** res = dereference(self._ptr).Data
        return ImFont.from_ptr(res)
    @data.setter
...
cdef class ImVector_ImFontAtlasCustomRect:
    cdef ccimgui_dear_bindings.ImVector_ImFontAtlasCustomRect* _ptr
    
    @staticmethod
    cdef ImVector_ImFontAtlasCustomRect from_ptr(ccimgui_dear_bindings.ImVector_ImFontAtlasCustomRect* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImFontAtlasCustomRect* res = dereference(self._ptr).Data
...
cdef class ImVector_ImFontConfig:
    cdef ccimgui_dear_bindings.ImVector_ImFontConfig* _ptr
    
    @staticmethod
    cdef ImVector_ImFontConfig from_ptr(ccimgui_dear_bindings.ImVector_ImFontConfig* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImFontConfig* res = dereference(self._ptr).Data
...
cdef class ImVector_float:
    cdef ccimgui_dear_bindings.ImVector_float* _ptr
    
    @staticmethod
    cdef ImVector_float from_ptr(ccimgui_dear_bindings.ImVector_float* _ptr):
...
cdef class ImVector_ImFontGlyph:
    cdef ccimgui_dear_bindings.ImVector_ImFontGlyph* _ptr
    
    @staticmethod
    cdef ImVector_ImFontGlyph from_ptr(ccimgui_dear_bindings.ImVector_ImFontGlyph* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImFontGlyph* res = dereference(self._ptr).Data
...
cdef class ImVector_ImGuiPlatformMonitor:
    cdef ccimgui_dear_bindings.ImVector_ImGuiPlatformMonitor* _ptr
    
    @staticmethod
    cdef ImVector_ImGuiPlatformMonitor from_ptr(ccimgui_dear_bindings.ImVector_ImGuiPlatformMonitor* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImGuiPlatformMonitor* res = dereference(self._ptr).Data
...
cdef class ImVector_ImGuiViewportPtr:
    cdef ccimgui_dear_bindings.ImVector_ImGuiViewportPtr* _ptr
    
    @staticmethod
    cdef ImVector_ImGuiViewportPtr from_ptr(ccimgui_dear_bindings.ImVector_ImGuiViewportPtr* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImGuiViewport** res = dereference(self._ptr).Data
        return ImGuiViewport.from_ptr(res)
    @data.setter
...
cdef class ImGuiStyle:
    cdef ccimgui_dear_bindings.ImGuiStyle* _ptr
    
    @staticmethod
    cdef ImGuiStyle from_ptr(ccimgui_dear_bindings.ImGuiStyle* _ptr):
    ...
    @property
    def colors(self):
        cdef ccimgui_dear_bindings.ImVec4 res = dereference(self._ptr).Colors
        return _cast_ImVec4_tuple(res)
    @colors.setter
    ...
    def scale_all_sizes(self: ImGuiStyle, scale_factor: float):
        ccimgui_dear_bindings.ImGuiStyle_ScaleAllSizes(
            self._ptr,
            scale_factor
        )
    # [End Method]
...
cdef class ImGuiKeyData:
    ...
    @staticmethod
    cdef ImGuiKeyData from_ptr(ccimgui_dear_bindings.ImGuiKeyData* _ptr):
...
# ?use_template(False)
cdef class ImGuiIO:
    cdef ccimgui_dear_bindings.ImGuiIO* _ptr
    
    @staticmethod
    cdef ImGuiIO from_ptr(ccimgui_dear_bindings.ImGuiIO* _ptr):
...
cdef class ImGuiInputTextCallbackData:
    ...
    @staticmethod
    cdef ImGuiInputTextCallbackData from_ptr(ccimgui_dear_bindings.ImGuiInputTextCallbackData* _ptr):
    ...
    def clear_selection(self: ImGuiInputTextCallbackData):
        ccimgui_dear_bindings.ImGuiInputTextCallbackData_ClearSelection(
            self._ptr
        )
    # [End Method]
    ...
    def delete_chars(self: ImGuiInputTextCallbackData, pos: int, bytes_count: int):
        ccimgui_dear_bindings.ImGuiInputTextCallbackData_DeleteChars(
            self._ptr,
            pos,
            bytes_count
        )
    ...
    def has_selection(self: ImGuiInputTextCallbackData):
        cdef bool res = ccimgui_dear_bindings.ImGuiInputTextCallbackData_HasSelection(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def insert_chars(self: ImGuiInputTextCallbackData, pos: int, text: str, text_end: str=None):
        ccimgui_dear_bindings.ImGuiInputTextCallbackData_InsertChars(
            self._ptr,
            pos,
            _bytes(text),
            _bytes(text_end)
    ...
    def select_all(self: ImGuiInputTextCallbackData):
        ccimgui_dear_bindings.ImGuiInputTextCallbackData_SelectAll(
            self._ptr
        )
    # [End Method]
...
cdef class ImGuiSizeCallbackData:
    ...
    @staticmethod
    cdef ImGuiSizeCallbackData from_ptr(ccimgui_dear_bindings.ImGuiSizeCallbackData* _ptr):
...
cdef class ImGuiWindowClass:
    ...
    @staticmethod
    cdef ImGuiWindowClass from_ptr(ccimgui_dear_bindings.ImGuiWindowClass* _ptr):
...
cdef class ImGuiPayload:
    ...
    @staticmethod
    cdef ImGuiPayload from_ptr(ccimgui_dear_bindings.ImGuiPayload* _ptr):
    ...
    def clear(self: ImGuiPayload):
        ccimgui_dear_bindings.ImGuiPayload_Clear(
            self._ptr
        )
    # [End Method]
    ...
    def is_data_type(self: ImGuiPayload, type_: str):
        cdef bool res = ccimgui_dear_bindings.ImGuiPayload_IsDataType(
            self._ptr,
            _bytes(type_)
        )
        return res
    ...
    def is_delivery(self: ImGuiPayload):
        cdef bool res = ccimgui_dear_bindings.ImGuiPayload_IsDelivery(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def is_preview(self: ImGuiPayload):
        cdef bool res = ccimgui_dear_bindings.ImGuiPayload_IsPreview(
            self._ptr
        )
        return res
    # [End Method]
...
cdef class ImGuiTableColumnSortSpecs:
    ...
    @staticmethod
    cdef ImGuiTableColumnSortSpecs from_ptr(ccimgui_dear_bindings.ImGuiTableColumnSortSpecs* _ptr):
...
cdef class ImGuiTableSortSpecs:
    ...
    @staticmethod
    cdef ImGuiTableSortSpecs from_ptr(ccimgui_dear_bindings.ImGuiTableSortSpecs* _ptr):
...
cdef class ImGuiTextFilter_ImGuiTextRange:
    ...
    @staticmethod
    cdef ImGuiTextFilter_ImGuiTextRange from_ptr(ccimgui_dear_bindings.ImGuiTextFilter_ImGuiTextRange* _ptr):
    ...
    def empty(self: ImGuiTextFilter_ImGuiTextRange):
        cdef bool res = ccimgui_dear_bindings.ImGuiTextFilter_ImGuiTextRange_empty(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def split(self: ImGuiTextFilter_ImGuiTextRange, separator: int, out: ImVector_ImGuiTextFilter_ImGuiTextRange):
        ccimgui_dear_bindings.ImGuiTextFilter_ImGuiTextRange_split(
            self._ptr,
            separator,
            out
        )
...
cdef class ImGuiTextFilter:
    ...
    @staticmethod
    cdef ImGuiTextFilter from_ptr(ccimgui_dear_bindings.ImGuiTextFilter* _ptr):
    ...
    @property
    def filters(self):
        cdef ccimgui_dear_bindings.ImVector_ImGuiTextFilter_ImGuiTextRange res = dereference(self._ptr).Filters
    ...
    def build(self: ImGuiTextFilter):
        ccimgui_dear_bindings.ImGuiTextFilter_Build(
            self._ptr
        )
    # [End Method]
    ...
    def clear(self: ImGuiTextFilter):
        ccimgui_dear_bindings.ImGuiTextFilter_Clear(
            self._ptr
        )
    # [End Method]
    ...
    def is_active(self: ImGuiTextFilter):
        cdef bool res = ccimgui_dear_bindings.ImGuiTextFilter_IsActive(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def pass_filter(self: ImGuiTextFilter, text: str, text_end: str=None):
        cdef bool res = ccimgui_dear_bindings.ImGuiTextFilter_PassFilter(
            self._ptr,
            _bytes(text),
            _bytes(text_end)
        )
...
cdef class ImGuiTextBuffer:
    ...
    @staticmethod
    cdef ImGuiTextBuffer from_ptr(ccimgui_dear_bindings.ImGuiTextBuffer* _ptr):
    ...
    @property
    def buf(self):
        cdef ccimgui_dear_bindings.ImVector_char res = dereference(self._ptr).Buf
    ...
    def append(self: ImGuiTextBuffer, str_: str, str_end: str=None):
        ccimgui_dear_bindings.ImGuiTextBuffer_append(
            self._ptr,
            _bytes(str_),
            _bytes(str_end)
        )
    ...
    def appendf(self: ImGuiTextBuffer, fmt: str):
        ccimgui_dear_bindings.ImGuiTextBuffer_appendf(
            self._ptr,
            _bytes(fmt)
        )
    # [End Method]
    ...
    def appendfv(self: ImGuiTextBuffer, fmt: str):
        ccimgui_dear_bindings.ImGuiTextBuffer_appendfv(
            self._ptr,
            _bytes(fmt)
        )
    # [End Method]
    ...
    def begin(self: ImGuiTextBuffer):
        cdef const char* res = ccimgui_dear_bindings.ImGuiTextBuffer_begin(
            self._ptr
        )
        return _from_bytes(res)
    # [End Method]
    ...
    def cstr(self: ImGuiTextBuffer):
        cdef const char* res = ccimgui_dear_bindings.ImGuiTextBuffer_c_str(
            self._ptr
        )
        return _from_bytes(res)
    # [End Method]
    ...
    def clear(self: ImGuiTextBuffer):
        ccimgui_dear_bindings.ImGuiTextBuffer_clear(
            self._ptr
        )
    # [End Method]
    ...
    def empty(self: ImGuiTextBuffer):
        cdef bool res = ccimgui_dear_bindings.ImGuiTextBuffer_empty(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def reserve(self: ImGuiTextBuffer, capacity: int):
        ccimgui_dear_bindings.ImGuiTextBuffer_reserve(
            self._ptr,
            capacity
        )
    # [End Method]
    ...
    def size(self: ImGuiTextBuffer):
        cdef int res = ccimgui_dear_bindings.ImGuiTextBuffer_size(
            self._ptr
        )
        return res
    # [End Method]
...
cdef class ImGuiStorage_ImGuiStoragePair:
    ...
    @staticmethod
    cdef ImGuiStorage_ImGuiStoragePair from_ptr(ccimgui_dear_bindings.ImGuiStorage_ImGuiStoragePair* _ptr):
    ...
    @property
    def key(self):
        cdef ccimgui_dear_bindings.ImGuiID res = dereference(self._ptr).key
        return res
    @key.setter
...
cdef class ImGuiStorage:
    ...
    @staticmethod
    cdef ImGuiStorage from_ptr(ccimgui_dear_bindings.ImGuiStorage* _ptr):
    ...
    @property
    def data(self):
        cdef ccimgui_dear_bindings.ImVector_ImGuiStorage_ImGuiStoragePair res = dereference(self._ptr).Data
    ...
    def get_bool(self: ImGuiStorage, key: int, default_val: bool=False):
        cdef bool res = ccimgui_dear_bindings.ImGuiStorage_GetBool(
            self._ptr,
            key,
            default_val
        )
    ...
    def get_bool_ref(self: ImGuiStorage, key: int, default_val: bool=False):
        cdef bool* res = ccimgui_dear_bindings.ImGuiStorage_GetBoolRef(
            self._ptr,
            key,
            default_val
        )
    ...
    def get_float(self: ImGuiStorage, key: int, default_val: float=0.0):
        cdef float res = ccimgui_dear_bindings.ImGuiStorage_GetFloat(
            self._ptr,
            key,
            default_val
        )
    ...
    def get_float_ref(self: ImGuiStorage, key: int, default_val: float=0.0):
        cdef float* res = ccimgui_dear_bindings.ImGuiStorage_GetFloatRef(
            self._ptr,
            key,
            default_val
        )
    ...
    def get_int(self: ImGuiStorage, key: int, default_val: int=0):
        cdef int res = ccimgui_dear_bindings.ImGuiStorage_GetInt(
            self._ptr,
            key,
            default_val
        )
    ...
    def get_void_ptr_ref(self: ImGuiStorage, key: int, default_val: Any=None):
        cdef void** res = ccimgui_dear_bindings.ImGuiStorage_GetVoidPtrRef(
            self._ptr,
            key,
            default_val
        )
    ...
    def set_bool(self: ImGuiStorage, key: int, val: bool):
        ccimgui_dear_bindings.ImGuiStorage_SetBool(
            self._ptr,
            key,
            val
        )
    ...
    def set_float(self: ImGuiStorage, key: int, val: float):
        ccimgui_dear_bindings.ImGuiStorage_SetFloat(
            self._ptr,
            key,
            val
        )
    ...
    def set_int(self: ImGuiStorage, key: int, val: int):
        ccimgui_dear_bindings.ImGuiStorage_SetInt(
            self._ptr,
            key,
            val
        )
    ...
    def set_void_ptr(self: ImGuiStorage, key: int, val: Any):
        ccimgui_dear_bindings.ImGuiStorage_SetVoidPtr(
            self._ptr,
            key,
            val
        )
...
cdef class ImGuiListClipper:
    ...
    @staticmethod
    cdef ImGuiListClipper from_ptr(ccimgui_dear_bindings.ImGuiListClipper* _ptr):
    ...
    def begin(self: ImGuiListClipper, items_count: int, items_height: float=-1.0):
        ccimgui_dear_bindings.ImGuiListClipper_Begin(
            self._ptr,
            items_count,
            items_height
        )
...
cdef class ImColor:
    ...
    @staticmethod
    cdef ImColor from_ptr(ccimgui_dear_bindings.ImColor* _ptr):
    ...
    @property
    def value(self):
        cdef ccimgui_dear_bindings.ImVec4 res = dereference(self._ptr).Value
        return _cast_ImVec4_tuple(res)
    @value.setter
    ...
    def hsv(self: ImColor, h: float, s: float, v: float, a: float=1.0):
        cdef ccimgui_dear_bindings.ImColor res = ccimgui_dear_bindings.ImColor_HSV(
            self._ptr,
            h,
            s,
            v,
...
cdef class ImDrawCmd:
    ...
    @staticmethod
    cdef ImDrawCmd from_ptr(ccimgui_dear_bindings.ImDrawCmd* _ptr):
...
cdef class ImDrawVert:
    cdef ccimgui_dear_bindings.ImDrawVert* _ptr
    
    @staticmethod
    cdef ImDrawVert from_ptr(ccimgui_dear_bindings.ImDrawVert* _ptr):
    ...
    @property
    def uv(self):
        cdef ccimgui_dear_bindings.ImVec2 res = dereference(self._ptr).uv
        return _cast_ImVec2_tuple(res)
    @uv.setter
    ...
    @property
    def col(self):
        cdef ccimgui_dear_bindings.ImU32 res = dereference(self._ptr).col
        return res
    @col.setter
...
cdef class ImDrawCmdHeader:
    ...
    @staticmethod
    cdef ImDrawCmdHeader from_ptr(ccimgui_dear_bindings.ImDrawCmdHeader* _ptr):
...
cdef class ImDrawChannel:
    ...
    @staticmethod
    cdef ImDrawChannel from_ptr(ccimgui_dear_bindings.ImDrawChannel* _ptr):
    ...
    @property
    def cmd_buffer(self):
        cdef ccimgui_dear_bindings.ImVector_ImDrawCmd res = dereference(self._ptr)._CmdBuffer
    ...
    @property
    def idx_buffer(self):
        cdef ccimgui_dear_bindings.ImVector_ImDrawIdx res = dereference(self._ptr)._IdxBuffer
...
cdef class ImDrawListSplitter:
    ...
    @staticmethod
    cdef ImDrawListSplitter from_ptr(ccimgui_dear_bindings.ImDrawListSplitter* _ptr):
    ...
    def clear_free_memory(self: ImDrawListSplitter):
        ccimgui_dear_bindings.ImDrawListSplitter_ClearFreeMemory(
            self._ptr
        )
    # [End Method]
    ...
    def merge(self: ImDrawListSplitter, draw_list: ImDrawList):
        ccimgui_dear_bindings.ImDrawListSplitter_Merge(
            self._ptr,
            draw_list
        )
    # [End Method]
    ...
    def set_current_channel(self: ImDrawListSplitter, draw_list: ImDrawList, channel_idx: int):
        ccimgui_dear_bindings.ImDrawListSplitter_SetCurrentChannel(
            self._ptr,
            draw_list,
            channel_idx
        )
    ...
    def split(self: ImDrawListSplitter, draw_list: ImDrawList, count: int):
        ccimgui_dear_bindings.ImDrawListSplitter_Split(
            self._ptr,
            draw_list,
            count
        )
...
cdef class ImDrawList:
    ...
    @staticmethod
    cdef ImDrawList from_ptr(ccimgui_dear_bindings.ImDrawList* _ptr):
    ...
    def add_circle_ex(self: ImDrawList, center: tuple, radius: float, col: int, num_segments: int=0, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_AddCircleEx(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            col,
    ...
    def add_circle_filled(self: ImDrawList, center: tuple, radius: float, col: int, num_segments: int=0):
        ccimgui_dear_bindings.ImDrawList_AddCircleFilled(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            col,
    ...
    def add_convex_poly_filled(self: ImDrawList, points: ImVec2, num_points: int, col: int):
        ccimgui_dear_bindings.ImDrawList_AddConvexPolyFilled(
            self._ptr,
            points,
            num_points,
            col
    ...
    def add_image_ex(self: ImDrawList, user_texture_id: Any, p_min: tuple, p_max: tuple, uv_min: tuple=(0, 0), uv_max: tuple=(1, 1), col: int=IM_COL32_WHITE):
        ccimgui_dear_bindings.ImDrawList_AddImageEx(
            self._ptr,
            user_texture_id,
            _cast_tuple_ImVec2(p_min),
            _cast_tuple_ImVec2(p_max),
    ...
    def add_image_quad_ex(self: ImDrawList, user_texture_id: Any, p1: tuple, p2: tuple, p3: tuple, p4: tuple, uv1: tuple=(0, 0), uv2: tuple=(1, 0), uv3: tuple=(1, 1), uv4: tuple=(0, 1), col: int=IM_COL32_WHITE):
        ccimgui_dear_bindings.ImDrawList_AddImageQuadEx(
            self._ptr,
            user_texture_id,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
    ...
    def add_image_rounded(self: ImDrawList, user_texture_id: Any, p_min: tuple, p_max: tuple, uv_min: tuple, uv_max: tuple, col: int, rounding: float, flags: int=0):
        ccimgui_dear_bindings.ImDrawList_AddImageRounded(
            self._ptr,
            user_texture_id,
            _cast_tuple_ImVec2(p_min),
            _cast_tuple_ImVec2(p_max),
    ...
    def add_line_ex(self: ImDrawList, p1: tuple, p2: tuple, col: int, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_AddLineEx(
            self._ptr,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
            col,
    ...
    def add_ngon_ex(self: ImDrawList, center: tuple, radius: float, col: int, num_segments: int, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_AddNgonEx(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            col,
    ...
    def add_ngon_filled(self: ImDrawList, center: tuple, radius: float, col: int, num_segments: int):
        ccimgui_dear_bindings.ImDrawList_AddNgonFilled(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            col,
    ...
    def add_polyline(self: ImDrawList, points: ImVec2, num_points: int, col: int, flags: int, thickness: float):
        ccimgui_dear_bindings.ImDrawList_AddPolyline(
            self._ptr,
            points,
            num_points,
            col,
    ...
    def add_quad_ex(self: ImDrawList, p1: tuple, p2: tuple, p3: tuple, p4: tuple, col: int, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_AddQuadEx(
            self._ptr,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
            _cast_tuple_ImVec2(p3),
    ...
    def add_quad_filled(self: ImDrawList, p1: tuple, p2: tuple, p3: tuple, p4: tuple, col: int):
        ccimgui_dear_bindings.ImDrawList_AddQuadFilled(
            self._ptr,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
            _cast_tuple_ImVec2(p3),
    ...
    def add_rect_filled_multi_color(self: ImDrawList, p_min: tuple, p_max: tuple, col_upr_left: int, col_upr_right: int, col_bot_right: int, col_bot_left: int):
        ccimgui_dear_bindings.ImDrawList_AddRectFilledMultiColor(
            self._ptr,
            _cast_tuple_ImVec2(p_min),
            _cast_tuple_ImVec2(p_max),
            col_upr_left,
    ...
    def add_text_ex(self: ImDrawList, pos: tuple, col: int, text_begin: str, text_end: str=None):
        ccimgui_dear_bindings.ImDrawList_AddTextEx(
            self._ptr,
            _cast_tuple_ImVec2(pos),
            col,
            _bytes(text_begin),
    ...
    def add_text_im_font_ptr_ex(self: ImDrawList, font: ImFont, font_size: float, pos: tuple, col: int, text_begin: str, text_end: str=None, wrap_width: float=0.0, cpu_fine_clip_rect: ImVec4=None):
        ccimgui_dear_bindings.ImDrawList_AddTextImFontPtrEx(
            self._ptr,
            font,
            font_size,
            _cast_tuple_ImVec2(pos),
    ...
    def add_triangle_ex(self: ImDrawList, p1: tuple, p2: tuple, p3: tuple, col: int, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_AddTriangleEx(
            self._ptr,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
            _cast_tuple_ImVec2(p3),
    ...
    def add_triangle_filled(self: ImDrawList, p1: tuple, p2: tuple, p3: tuple, col: int):
        ccimgui_dear_bindings.ImDrawList_AddTriangleFilled(
            self._ptr,
            _cast_tuple_ImVec2(p1),
            _cast_tuple_ImVec2(p2),
            _cast_tuple_ImVec2(p3),
    ...
    def channels_merge(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList_ChannelsMerge(
            self._ptr
        )
    # [End Method]
    ...
    def channels_set_current(self: ImDrawList, n: int):
        ccimgui_dear_bindings.ImDrawList_ChannelsSetCurrent(
            self._ptr,
            n
        )
    # [End Method]
    ...
    def get_clip_rect_max(self: ImDrawList):
        cdef ccimgui_dear_bindings.ImVec2 res = ccimgui_dear_bindings.ImDrawList_GetClipRectMax(
            self._ptr
        )
        return _cast_ImVec2_tuple(res)
    # [End Method]
    ...
    def get_clip_rect_min(self: ImDrawList):
        cdef ccimgui_dear_bindings.ImVec2 res = ccimgui_dear_bindings.ImDrawList_GetClipRectMin(
            self._ptr
        )
        return _cast_ImVec2_tuple(res)
    # [End Method]
    ...
    def path_arc_to(self: ImDrawList, center: tuple, radius: float, a_min: float, a_max: float, num_segments: int=0):
        ccimgui_dear_bindings.ImDrawList_PathArcTo(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            a_min,
    ...
    def path_fill_convex(self: ImDrawList, col: int):
        ccimgui_dear_bindings.ImDrawList_PathFillConvex(
            self._ptr,
            col
        )
    # [End Method]
    ...
    def path_line_to(self: ImDrawList, pos: tuple):
        ccimgui_dear_bindings.ImDrawList_PathLineTo(
            self._ptr,
            _cast_tuple_ImVec2(pos)
        )
    # [End Method]
    ...
    def path_line_to_merge_duplicate(self: ImDrawList, pos: tuple):
        ccimgui_dear_bindings.ImDrawList_PathLineToMergeDuplicate(
            self._ptr,
            _cast_tuple_ImVec2(pos)
        )
    # [End Method]
    ...
    def path_rect(self: ImDrawList, rect_min: tuple, rect_max: tuple, rounding: float=0.0, flags: int=0):
        ccimgui_dear_bindings.ImDrawList_PathRect(
            self._ptr,
            _cast_tuple_ImVec2(rect_min),
            _cast_tuple_ImVec2(rect_max),
            rounding,
    ...
    def path_stroke(self: ImDrawList, col: int, flags: int=0, thickness: float=1.0):
        ccimgui_dear_bindings.ImDrawList_PathStroke(
            self._ptr,
            col,
            flags,
            thickness
    ...
    def pop_clip_rect(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList_PopClipRect(
            self._ptr
        )
    # [End Method]
    ...
    def pop_texture_id(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList_PopTextureID(
            self._ptr
        )
    # [End Method]
    ...
    def prim_quad_uv(self: ImDrawList, a: tuple, b: tuple, c: tuple, d: tuple, uv_a: tuple, uv_b: tuple, uv_c: tuple, uv_d: tuple, col: int):
        ccimgui_dear_bindings.ImDrawList_PrimQuadUV(
            self._ptr,
    ...
    def prim_rect_uv(self: ImDrawList, a: tuple, b: tuple, uv_a: tuple, uv_b: tuple, col: int):
        ccimgui_dear_bindings.ImDrawList_PrimRectUV(
            self._ptr,
            _cast_tuple_ImVec2(a),
            _cast_tuple_ImVec2(b),
            _cast_tuple_ImVec2(uv_a),
    ...
    def prim_unreserve(self: ImDrawList, idx_count: int, vtx_count: int):
        ccimgui_dear_bindings.ImDrawList_PrimUnreserve(
            self._ptr,
            idx_count,
            vtx_count
        )
    ...
    def prim_write_idx(self: ImDrawList, idx: int):
        ccimgui_dear_bindings.ImDrawList_PrimWriteIdx(
            self._ptr,
            idx
        )
    # [End Method]
    ...
    def prim_write_vtx(self: ImDrawList, pos: tuple, uv: tuple, col: int):
        ccimgui_dear_bindings.ImDrawList_PrimWriteVtx(
            self._ptr,
            _cast_tuple_ImVec2(pos),
            _cast_tuple_ImVec2(uv),
            col
    ...
    def push_clip_rect_full_screen(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList_PushClipRectFullScreen(
            self._ptr
        )
    # [End Method]
    ...
    def push_texture_id(self: ImDrawList, texture_id: Any):
        ccimgui_dear_bindings.ImDrawList_PushTextureID(
            self._ptr,
            texture_id
        )
    # [End Method]
    ...
    def calc_circle_auto_segment_count(self: ImDrawList, radius: float):
        cdef int res = ccimgui_dear_bindings.ImDrawList__CalcCircleAutoSegmentCount(
            self._ptr,
            radius
        )
        return res
    ...
    def clear_free_memory(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__ClearFreeMemory(
            self._ptr
        )
    # [End Method]
    ...
    def on_changed_clip_rect(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__OnChangedClipRect(
            self._ptr
        )
    # [End Method]
    ...
    def on_changed_texture_id(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__OnChangedTextureID(
            self._ptr
        )
    # [End Method]
    ...
    def on_changed_vtx_offset(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__OnChangedVtxOffset(
            self._ptr
        )
    # [End Method]
    ...
    def path_arc_to_fast_ex(self: ImDrawList, center: tuple, radius: float, a_min_sample: int, a_max_sample: int, a_step: int):
        ccimgui_dear_bindings.ImDrawList__PathArcToFastEx(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            a_min_sample,
    ...
    def path_arc_to_n(self: ImDrawList, center: tuple, radius: float, a_min: float, a_max: float, num_segments: int):
        ccimgui_dear_bindings.ImDrawList__PathArcToN(
            self._ptr,
            _cast_tuple_ImVec2(center),
            radius,
            a_min,
    ...
    def pop_unused_draw_cmd(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__PopUnusedDrawCmd(
            self._ptr
        )
    # [End Method]
    ...
    def try_merge_draw_cmds(self: ImDrawList):
        ccimgui_dear_bindings.ImDrawList__TryMergeDrawCmds(
            self._ptr
        )
    # [End Method]
...
cdef class ImDrawData:
    ...
    @staticmethod
    cdef ImDrawData from_ptr(ccimgui_dear_bindings.ImDrawData* _ptr):
...
cdef class ImFontConfig:
    cdef ccimgui_dear_bindings.ImFontConfig* _ptr
    
    @staticmethod
    cdef ImFontConfig from_ptr(ccimgui_dear_bindings.ImFontConfig* _ptr):
    ...
    @property
    def dst_font(self):
        cdef ccimgui_dear_bindings.ImFont* res = dereference(self._ptr).DstFont
        return ImFont.from_ptr(res)
    @dst_font.setter
...
cdef class ImFontGlyph:
    ...
    @staticmethod
    cdef ImFontGlyph from_ptr(ccimgui_dear_bindings.ImFontGlyph* _ptr):
...
cdef class ImFontGlyphRangesBuilder:
    ...
    @staticmethod
    cdef ImFontGlyphRangesBuilder from_ptr(ccimgui_dear_bindings.ImFontGlyphRangesBuilder* _ptr):
    ...
    def clear(self: ImFontGlyphRangesBuilder):
        ccimgui_dear_bindings.ImFontGlyphRangesBuilder_Clear(
            self._ptr
        )
    # [End Method]
...
cdef class ImFontAtlasCustomRect:
    ...
    @staticmethod
    cdef ImFontAtlasCustomRect from_ptr(ccimgui_dear_bindings.ImFontAtlasCustomRect* _ptr):
    ...
    def is_packed(self: ImFontAtlasCustomRect):
        cdef bool res = ccimgui_dear_bindings.ImFontAtlasCustomRect_IsPacked(
            self._ptr
        )
        return res
    # [End Method]
...
cdef class ImFontAtlas:
    ...
    @staticmethod
    cdef ImFontAtlas from_ptr(ccimgui_dear_bindings.ImFontAtlas* _ptr):
    ...
    def add_custom_rect_font_glyph(self: ImFontAtlas, font: ImFont, id_: int, width: int, height: int, advance_x: float, offset: tuple=(0, 0)):
        cdef int res = ccimgui_dear_bindings.ImFontAtlas_AddCustomRectFontGlyph(
            self._ptr,
            font,
            id_,
            width,
    ...
    def add_font(self: ImFontAtlas, font_cfg: ImFontConfig):
        cdef ccimgui_dear_bindings.ImFont* res = ccimgui_dear_bindings.ImFontAtlas_AddFont(
            self._ptr,
            font_cfg
        )
        return ImFont.from_ptr(res)
    ...
    def add_font_default(self: ImFontAtlas, font_cfg: ImFontConfig=None):
        cdef ccimgui_dear_bindings.ImFont* res = ccimgui_dear_bindings.ImFontAtlas_AddFontDefault(
            self._ptr,
            font_cfg
        )
        return ImFont.from_ptr(res)
    ...
    def add_font_from_file_ttf(self: ImFontAtlas, filename: str, size_pixels: float, font_cfg: ImFontConfig=None, glyph_ranges: int=None):
        cdef ccimgui_dear_bindings.ImFont* res = ccimgui_dear_bindings.ImFontAtlas_AddFontFromFileTTF(
            self._ptr,
            _bytes(filename),
            size_pixels,
            font_cfg,
    ...
    def get_custom_rect_by_index(self: ImFontAtlas, index: int):
        cdef ccimgui_dear_bindings.ImFontAtlasCustomRect* res = ccimgui_dear_bindings.ImFontAtlas_GetCustomRectByIndex(
            self._ptr,
            index
        )
        return ImFontAtlasCustomRect.from_ptr(res)
    ...
    def get_mouse_cursor_tex_data(self: ImFontAtlas, cursor: int, out_offset: ImVec2, out_size: ImVec2, out_uv_border: tuple, out_uv_fill: tuple):
        cdef bool res = ccimgui_dear_bindings.ImFontAtlas_GetMouseCursorTexData(
            self._ptr,
            cursor,
            out_offset,
            out_size,
    ...
    def set_tex_id(self: ImFontAtlas, id_: Any):
        ccimgui_dear_bindings.ImFontAtlas_SetTexID(
            self._ptr,
            id_
        )
    # [End Method]
...
cdef class ImFont:
    ...
    @staticmethod
    cdef ImFont from_ptr(ccimgui_dear_bindings.ImFont* _ptr):
    ...
    def add_glyph(self: ImFont, src_cfg: ImFontConfig, c: int, x0: float, y0: float, x1: float, y1: float, u0: float, v0: float, u1: float, v1: float, advance_x: float):
        ccimgui_dear_bindings.ImFont_AddGlyph(
            self._ptr,
            src_cfg,
            c,
            x0,
    ...
    def calc_word_wrap_position_a(self: ImFont, scale: float, text: str, text_end: str, wrap_width: float):
        cdef const char* res = ccimgui_dear_bindings.ImFont_CalcWordWrapPositionA(
            self._ptr,
            scale,
            _bytes(text),
            _bytes(text_end),
    ...
    def clear_output_data(self: ImFont):
        ccimgui_dear_bindings.ImFont_ClearOutputData(
            self._ptr
        )
    # [End Method]
    ...
    def find_glyph(self: ImFont, c: int):
        cdef ccimgui_dear_bindings.ImFontGlyph* res = ccimgui_dear_bindings.ImFont_FindGlyph(
            self._ptr,
            c
        )
        return ImFontGlyph.from_ptr(res)
    ...
    def find_glyph_no_fallback(self: ImFont, c: int):
        cdef ccimgui_dear_bindings.ImFontGlyph* res = ccimgui_dear_bindings.ImFont_FindGlyphNoFallback(
            self._ptr,
            c
        )
        return ImFontGlyph.from_ptr(res)
    ...
    def get_char_advance(self: ImFont, c: int):
        cdef float res = ccimgui_dear_bindings.ImFont_GetCharAdvance(
            self._ptr,
            c
        )
        return res
    ...
    def get_debug_name(self: ImFont):
        cdef const char* res = ccimgui_dear_bindings.ImFont_GetDebugName(
            self._ptr
        )
        return _from_bytes(res)
    # [End Method]
    ...
    def grow_index(self: ImFont, new_size: int):
        ccimgui_dear_bindings.ImFont_GrowIndex(
            self._ptr,
            new_size
        )
    # [End Method]
    ...
    def is_glyph_range_unused(self: ImFont, c_begin: int, c_last: int):
        cdef bool res = ccimgui_dear_bindings.ImFont_IsGlyphRangeUnused(
            self._ptr,
            c_begin,
            c_last
        )
    ...
    def is_loaded(self: ImFont):
        cdef bool res = ccimgui_dear_bindings.ImFont_IsLoaded(
            self._ptr
        )
        return res
    # [End Method]
    ...
    def render_char(self: ImFont, draw_list: ImDrawList, size: float, pos: tuple, col: int, c: int):
        ccimgui_dear_bindings.ImFont_RenderChar(
            self._ptr,
            draw_list,
            size,
            _cast_tuple_ImVec2(pos),
    ...
    def render_text(self: ImFont, draw_list: ImDrawList, size: float, pos: tuple, col: int, clip_rect: tuple, text_begin: str, text_end: str, wrap_width: float=0.0, cpu_fine_clip: bool=False):
        ccimgui_dear_bindings.ImFont_RenderText(
            self._ptr,
            draw_list,
            size,
            _cast_tuple_ImVec2(pos),
    ...
    def set_glyph_visible(self: ImFont, c: int, visible: bool):
        ccimgui_dear_bindings.ImFont_SetGlyphVisible(
            self._ptr,
            c,
            visible
        )
...
cdef class ImGuiViewport:
    ...
    @staticmethod
    cdef ImGuiViewport from_ptr(ccimgui_dear_bindings.ImGuiViewport* _ptr):
    ...
    def get_work_center(self: ImGuiViewport):
        cdef ccimgui_dear_bindings.ImVec2 res = ccimgui_dear_bindings.ImGuiViewport_GetWorkCenter(
            self._ptr
        )
        return _cast_ImVec2_tuple(res)
    # [End Method]
...
cdef class ImGuiPlatformIO:
    ...
    @staticmethod
    cdef ImGuiPlatformIO from_ptr(ccimgui_dear_bindings.ImGuiPlatformIO* _ptr):
...
cdef class ImGuiPlatformMonitor:
    ...
    @staticmethod
    cdef ImGuiPlatformMonitor from_ptr(ccimgui_dear_bindings.ImGuiPlatformMonitor* _ptr):
...
cdef class ImGuiPlatformImeData:
    ...
    @staticmethod
    cdef ImGuiPlatformImeData from_ptr(ccimgui_dear_bindings.ImGuiPlatformImeData* _ptr):
